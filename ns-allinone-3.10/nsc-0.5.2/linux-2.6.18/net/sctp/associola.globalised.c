# 1 "linux-2.6.18/net/sctp/associola.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.18/net/sctp/associola.c"
# 51 "linux-2.6.18/net/sctp/associola.c"
...
# 52 "linux-2.6.18/net/sctp/associola.c" 2
...
# 53 "linux-2.6.18/net/sctp/associola.c" 2
...
# 10 "linux-2.6.18/include/linux/poll.h" 2
...
extern int do_sys_poll(struct pollfd * ufds, unsigned int nfds,
         s64 *timeout);
# 54 "linux-2.6.18/net/sctp/associola.c" 2
...
# 59 "linux-2.6.18/net/sctp/associola.c" 2
...
# 60 "linux-2.6.18/net/sctp/associola.c" 2
...
typedef  struct list_head   _GLOBAL_251_T; extern  _GLOBAL_251_T  global_tty_drivers[NUM_STACKS];    
...
typedef  struct termios   _GLOBAL_252_T; extern  _GLOBAL_252_T  global_tty_std_termios[NUM_STACKS];    

typedef  int  _GLOBAL_253_T; extern  _GLOBAL_253_T  global_kmsg_redirect[NUM_STACKS];   
...
typedef  struct mutex   _GLOBAL_254_T; extern  _GLOBAL_254_T  global_tty_mutex[NUM_STACKS];    


typedef  struct tty_ldisc   _GLOBAL_255_T; extern  _GLOBAL_255_T  global_tty_ldisc_N_TTY[NUM_STACKS];    
...
typedef  struct proc_dir_entry   _GLOBAL_256_T; extern  _GLOBAL_256_T  global_proc_root[NUM_STACKS];    
typedef  struct proc_dir_entry   _GLOBAL_257_T; extern  _GLOBAL_257_T  * global_proc_root_fs[NUM_STACKS];    
typedef  struct proc_dir_entry   _GLOBAL_258_T; extern  _GLOBAL_258_T  * global_proc_net[NUM_STACKS];    
typedef  struct proc_dir_entry   _GLOBAL_259_T; extern  _GLOBAL_259_T  * global_proc_net_stat[NUM_STACKS];    
typedef  struct proc_dir_entry   _GLOBAL_260_T; extern  _GLOBAL_260_T  * global_proc_bus[NUM_STACKS];    
typedef  struct proc_dir_entry   _GLOBAL_261_T; extern  _GLOBAL_261_T  * global_proc_root_driver[NUM_STACKS];    
typedef  struct proc_dir_entry   _GLOBAL_262_T; extern  _GLOBAL_262_T  * global_proc_root_kcore[NUM_STACKS];    

typedef  spinlock_t  _GLOBAL_263_T; extern  _GLOBAL_263_T  global_proc_subdir_lock[NUM_STACKS];   
...
typedef  struct vfsmount   _GLOBAL_264_T; extern  _GLOBAL_264_T  * global_proc_mnt[NUM_STACKS];    
...
typedef  const struct file_operations    _GLOBAL_265_T; extern  _GLOBAL_265_T  global_proc_kcore_operations[NUM_STACKS];     
typedef  const struct file_operations    _GLOBAL_266_T; extern  _GLOBAL_266_T  global_proc_kmsg_operations[NUM_STACKS];     
typedef  const struct file_operations    _GLOBAL_267_T; extern  _GLOBAL_267_T  global_ppc_htab_operations[NUM_STACKS];     
...
# 61 "linux-2.6.18/net/sctp/associola.c" 2
...
# 62 "linux-2.6.18/net/sctp/associola.c" 2
       ...
       gfp_t gfp)
{
 ...
 (__builtin_constant_p(0) ? (__builtin_constant_p((sizeof(struct sctp_association))) ? __constant_c_and_count_memset(((asoc)),((0x01010101UL*(unsigned char)(0))),((sizeof(struct sctp_association)))) : __constant_c_memset(((asoc)),((0x01010101UL*(unsigned char)(0))),((sizeof(struct sctp_association))))) : (__builtin_constant_p((sizeof(struct sctp_association))) ? __memset_generic((((asoc))),(((0))),(((sizeof(struct sctp_association))))) : __memset_generic(((asoc)),((0)),((sizeof(struct sctp_association))))));
 ...
 (((&asoc->base.refcnt)->counter) = (1));
 asoc->base.dead = 0;
 asoc->base.malloced = 0;


 sctp_bind_addr_init(&asoc->base.bind_addr, ep->base.bind_addr.port);
 do { *(&asoc->base.addr_lock) = (rwlock_t) { .raw_lock = { }, }; } while (0);
 ...
 asoc->max_burst = (global_sctp_globals[get_stack_id()].max_burst);
 ...
 for (i = SCTP_EVENT_TIMEOUT_NONE; i < (SCTP_EVENT_TIMEOUT_AUTOCLOSE + 1); ++i) {
  init_timer(&asoc->timers[i]);
  asoc->timers[i].function = (*_GLOBAL_sctp_timer_events_66_A[get_stack_id()])[i];
 ...
 asoc->peer.rwnd = 65535;


 asoc->sndbuf_used = 0;


 (((&asoc->rmem_alloc)->counter) = (0));
 ...
 asoc->unack_data = 0;
# 244 "linux-2.6.18/net/sctp/associola.c"
...
# 263 "linux-2.6.18/net/sctp/associola.c"
 ...
 sctp_tsnmap_init(&asoc->peer.tsn_map, 2048, 0);
      ...
      gfp_t gfp)
{
 ...
 asoc = (struct sctp_association *)kmalloc(sizeof(struct sctp_association), gfp);
 if (!asoc)
  goto fail;
...
void sctp_association_free(struct sctp_association *asoc)
{
 ...
 for (pos = (&asoc->peer.transport_addr_list)->next, temp = pos->next; pos != (&asoc->peer.transport_addr_list); pos = temp, temp = pos->next) {
...
static void sctp_association_destroy(struct sctp_association *asoc)
{
 ...
 if (asoc->assoc_id != 0) {
  do { local_bh_disable(); do { do { } while (0); (void)0; (void)(&global_sctp_assocs_id_lock[get_stack_id()]); } while (0); } while (0);
  idr_remove(&global_sctp_assocs_id[get_stack_id()], asoc->assoc_id);
  do { do { } while (0); local_bh_enable(); (void)0; (void)(&global_sctp_assocs_id_lock[get_stack_id()]); } while (0);
 }

 do { if (__builtin_expect(!!(!(!((&asoc->rmem_alloc)->counter))), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "!atomic_read(&asoc->rmem_alloc)", "linux-2.6.18/net/sctp/associola.c" , 423); } } while(0);
       ...
       struct sctp_transport *transport)
{
 ...
 (__builtin_constant_p(sizeof(union sctp_addr)) ? __constant_memcpy((&asoc->peer.primary_addr),(&transport->ipaddr),(sizeof(union sctp_addr))) : __memcpy((&asoc->peer.primary_addr),(&transport->ipaddr),(sizeof(union sctp_addr))));
...
# 459 "linux-2.6.18/net/sctp/associola.c"
   ...
   struct sctp_transport *peer)
{
 ...
 ;
# 489 "linux-2.6.18/net/sctp/associola.c"
        ...
        const int peer_state)
{
 ...
 if (asoc->pathmtu)
  asoc->pathmtu = ({ int __x = (peer->pathmtu); int __y = (asoc->pathmtu); __x < __y ? __x: __y; });
 else
 ...
 sctp_packet_init(&peer->packet, peer, asoc->base.bind_addr.port,
    asoc->peer.port);
# 621 "linux-2.6.18/net/sctp/associola.c"
 peer->cwnd = ({ typeof(4*asoc->pathmtu) _x = (4*asoc->pathmtu); typeof(({ __u32 __x = (2*asoc->pathmtu); __u32 __y = (4380); __x > __y ? __x: __y; })) _y = (({ __u32 __x = (2*asoc->pathmtu); __u32 __y = (4380); __x > __y ? __x: __y; })); (void) (&_x == &_y); _x < _y ? _x : _y; });
     ...
     const union sctp_addr *address)
{
 ...
 for (pos = (&asoc->peer.transport_addr_list)->next; prefetch(pos->next), pos != (&asoc->peer.transport_addr_list); pos = pos->next) {
  t = ({ const typeof( ((struct sctp_transport *)0)->transports ) *__mptr = (pos); (struct sctp_transport *)( (char *)__mptr - __builtin_offsetof(struct sctp_transport,transports) );});
...
{
 struct sctp_transport *t = ((void *)0);
 ...
 event = sctp_ulpevent_make_peer_addr_change(asoc,
    (struct __kernel_sockaddr_storage *) &transport->ipaddr,
    0, spc_state, error, ((( gfp_t)0x20u)));
 if (event)
  sctp_ulpq_tail_event(&asoc->ulpq, event);
# 741 "linux-2.6.18/net/sctp/associola.c"
...
# 767 "linux-2.6.18/net/sctp/associola.c"
          ...
          __u32 tsn)
{
 ...
 match = ((void *)0);
# 883 "linux-2.6.18/net/sctp/associola.c"
 active = asoc->peer.active_path;

 for (entry = (&active->transmitted)->next; prefetch(entry->next), entry != (&active->transmitted); entry = entry->next) {
  chunk = ({ const typeof( ((struct sctp_chunk *)0)->transmitted_list ) *__mptr = (entry); (struct sctp_chunk *)( (char *)__mptr - __builtin_offsetof(struct sctp_chunk,transmitted_list) );});
 ...
 for (pos = (&asoc->peer.transport_addr_list)->next; prefetch(pos->next), pos != (&asoc->peer.transport_addr_list); pos = pos->next) {
  ...
  if (transport == active)
   break;
  for (entry = (&transport->transmitted)->next; prefetch(entry->next), entry != (&transport->transmitted); entry = entry->next) {
   chunk = ({ const typeof( ((struct sctp_chunk *)0)->transmitted_list ) *__mptr = (entry); (struct sctp_chunk *)( (char *)__mptr - __builtin_offsetof(struct sctp_chunk,transmitted_list) );});
...
static void sctp_assoc_bh_rcv(struct sctp_association *asoc)
{
 ...
 while (((void *)0) != (chunk = sctp_inq_pop(inqueue))) {
  ...
  if ((chunk->chunk_hdr->type == SCTP_CID_DATA))
   ...
   (({ (void)(0); ((*_GLOBAL_sctp_statistics_65_A[get_stack_id()])[!(((current_thread_info()->preempt_count) & (((1UL << (8))-1) << (0 + 8))))]); })->mibs[SCTP_MIB_INCTRLCHUNKS]++);
  ...
  error = sctp_do_sm(SCTP_EVENT_T_CHUNK, subtype,
       state, ep, asoc, chunk, ((( gfp_t)0x20u)));
...
void sctp_assoc_migrate(struct sctp_association *assoc, struct sock *newsk)
{
 ...
 if (__sctp_style((oldsk), (SCTP_SOCKET_TCP)))
         ...
         struct sctp_association *new)
{
 ...
 sctp_tsnmap_init(&asoc->peer.tsn_map, 2048,
    asoc->peer.i.initial_tsn);


 for (pos = (&asoc->peer.transport_addr_list)->next, temp = pos->next; pos != (&asoc->peer.transport_addr_list); pos = temp, temp = pos->next) {
  trans = ({ const typeof( ((struct sctp_transport *)0)->transports ) *__mptr = (pos); (struct sctp_transport *)( (char *)__mptr - __builtin_offsetof(struct sctp_transport,transports) );});
 ...
 if (asoc->state >= SCTP_STATE_ESTABLISHED) {
  ...
  for (pos = (&new->peer.transport_addr_list)->next; prefetch(pos->next), pos != (&new->peer.transport_addr_list); pos = pos->next) {
   trans = ({ const typeof( ((struct sctp_transport *)0)->transports ) *__mptr = (pos); (struct sctp_transport *)( (char *)__mptr - __builtin_offsetof(struct sctp_transport,transports) );});

   if (!sctp_assoc_lookup_paddr(asoc, &trans->ipaddr))
    sctp_assoc_add_peer(asoc, &trans->ipaddr,
          ((( gfp_t)0x20u)), trans->state);
  }
...
static inline  __attribute__((always_inline)) int sctp_peer_needs_update(struct sctp_association *asoc)
{
 switch (asoc->state) {
 ...
 case SCTP_STATE_SHUTDOWN_SENT:
  if ((asoc->rwnd > asoc->a_rwnd) &&
      ((asoc->rwnd - asoc->a_rwnd) >=
       ({ __u32 __x = ((asoc->base.sk->sk_rcvbuf >> 1)); __u32 __y = (asoc->pathmtu); __x < __y ? __x: __y; })))
   return 1;
  break;
 default:
  break;
...
void sctp_assoc_rwnd_increase(struct sctp_association *asoc, unsigned len)
{
 ...
 ;
# 1253 "linux-2.6.18/net/sctp/associola.c"
         ...
         gfp_t gfp)
{
 ...
 flags = (10 == asoc->base.sk->__sk_common.skc_family) ? 0x00000001 : 0;
