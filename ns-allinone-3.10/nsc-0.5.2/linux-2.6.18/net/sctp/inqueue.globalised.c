# 1 "linux-2.6.18/net/sctp/inqueue.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.18/net/sctp/inqueue.c"
# 46 "linux-2.6.18/net/sctp/inqueue.c"
# 1 "linux-2.6.18/include/net/sctp/sctp.h" 1
# 73 "linux-2.6.18/include/net/sctp/sctp.h"
...
# 74 "linux-2.6.18/include/net/sctp/sctp.h" 2
...
# 47 "linux-2.6.18/net/sctp/inqueue.c" 2
...
# 48 "linux-2.6.18/net/sctp/inqueue.c" 2
...
void sctp_inq_init(struct sctp_inq *queue)
{
 ...
 do { INIT_LIST_HEAD(&(&queue->immediate)->entry); (&queue->immediate)->pending = 0; do { ((&queue->immediate))->func = (((void *)0)); ((&queue->immediate))->data = (((void *)0)); } while (0); init_timer(&(&queue->immediate)->timer); } while (0);
...
void sctp_inq_free(struct sctp_inq *queue)
{
 ...
 for (chunk = ({ const typeof( ((typeof(*chunk) *)0)->list ) *__mptr = ((&queue->in_chunk_list)->next); (typeof(*chunk) *)( (char *)__mptr - __builtin_offsetof(typeof(*chunk),list) );}), tmp = ({ const typeof( ((typeof(*chunk) *)0)->list ) *__mptr = (chunk->list.next); (typeof(*chunk) *)( (char *)__mptr - __builtin_offsetof(typeof(*chunk),list) );}); &chunk->list != (&queue->in_chunk_list); chunk = tmp, tmp = ({ const typeof( ((typeof(*tmp) *)0)->list ) *__mptr = (tmp->list.next); (typeof(*tmp) *)( (char *)__mptr - __builtin_offsetof(typeof(*tmp),list) );})) {
...
struct sctp_chunk *sctp_inq_pop(struct sctp_inq *queue)
{
 ...
 if ((chunk = queue->in_progress)) {
  ...
  if (chunk->singleton ||
      ...
      chunk->pdiscard) {
   sctp_chunk_free(chunk);
   chunk = queue->in_progress = ((void *)0);
  } else {
 ...
 if (!chunk) {
  ...
  chunk = queue->in_progress =
   ({ const typeof( ((struct sctp_chunk *)0)->list ) *__mptr = (entry); (struct sctp_chunk *)( (char *)__mptr - __builtin_offsetof(struct sctp_chunk,list) );});
  list_del_init(entry);
  ...
  chunk->data_accepted = 0;
        ...
        chunk->chunk_hdr = ch;
        chunk->chunk_end = ((__u8 *)ch) + (((ntohs(ch->length))+3)&~3);




 if (__builtin_expect(!!(skb_is_nonlinear(chunk->skb)), 0)) {
...
# 182 "linux-2.6.18/net/sctp/inqueue.c"
  sctp_chunk_free(chunk);
  chunk = queue->in_progress = ((void *)0);
...
# 208 "linux-2.6.18/net/sctp/inqueue.c"
void sctp_inq_set_th_handler(struct sctp_inq *q,
     void (*callback)(void *), void *arg)
{
 do { INIT_LIST_HEAD(&(&q->immediate)->entry); (&q->immediate)->pending = 0; do { ((&q->immediate))->func = (callback); ((&q->immediate))->data = (arg); } while (0); init_timer(&(&q->immediate)->timer); } while (0);
}
