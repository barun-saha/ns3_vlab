# 1 "linux-2.6.18/net/sctp/bind_addr.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.18/net/sctp/bind_addr.c"
# 45 "linux-2.6.18/net/sctp/bind_addr.c"
...
# 46 "linux-2.6.18/net/sctp/bind_addr.c" 2
...
# 47 "linux-2.6.18/net/sctp/bind_addr.c" 2
...
# 48 "linux-2.6.18/net/sctp/bind_addr.c" 2
# 1 "linux-2.6.18/include/net/sock.h" 1
# 46 "linux-2.6.18/include/net/sock.h"
...
# 49 "linux-2.6.18/net/sctp/bind_addr.c" 2
...
# 50 "linux-2.6.18/net/sctp/bind_addr.c" 2
...
# 52 "linux-2.6.18/net/sctp/bind_addr.c" 2
...
# 53 "linux-2.6.18/net/sctp/bind_addr.c" 2
   ...
   int flags)
{
 ...
 for (pos = (&src->address_list)->next; prefetch(pos->next), pos != (&src->address_list); pos = pos->next) {
...
static void sctp_bind_addr_clean(struct sctp_bind_addr *bp)
{
 ...
 for (pos = (&bp->address_list)->next, temp = pos->next; pos != (&bp->address_list); pos = temp, temp = pos->next) {
  addr = ({ const typeof( ((struct sctp_sockaddr_entry *)0)->list ) *__mptr = (pos); (struct sctp_sockaddr_entry *)( (char *)__mptr - __builtin_offsetof(struct sctp_sockaddr_entry,list) );});
  list_del(pos);
  kfree(addr);
  ;
         ...
         __u8 use_as_src, gfp_t gfp)
{
 ...
 addr = (struct sctp_sockaddr_entry *)kmalloc(sizeof(struct sctp_sockaddr_entry), gfp);
 if (!addr)
  return -12;

 (__builtin_constant_p(sizeof(*new)) ? __constant_memcpy((&addr->a),(new),(sizeof(*new))) : __memcpy((&addr->a),(new),(sizeof(*new))));
      ...
      gfp_t gfp)
{
 ...
 for (pos = (&bp->address_list)->next; prefetch(pos->next), pos != (&bp->address_list); pos = pos->next) {
  ...
  (__builtin_constant_p(len) ? __constant_memcpy((addrparms.v),(&rawaddr),(len)) : __memcpy((addrparms.v),(&rawaddr),(len)));
  addrparms.v += len;
  addrparms_len += len;
 }
    ...
    struct sctp_sock *opt)
{
 ...
 for (pos = (&bp->address_list)->next; prefetch(pos->next), pos != (&bp->address_list); pos = pos->next) {
  laddr = ({ const typeof( ((struct sctp_sockaddr_entry *)0)->list ) *__mptr = (pos); (struct sctp_sockaddr_entry *)( (char *)__mptr - __builtin_offsetof(struct sctp_sockaddr_entry,list) );});
         ...
         int flags)
{
 ...
 if (sctp_is_any(addr)) {
  ...
  if ((((2 == addr->sa.sa_family) &&
        (flags & 0x00000002))) ||
      (((10 == addr->sa.sa_family) &&
        (flags & 0x00000001) &&
        (flags & 0x00000004))))
