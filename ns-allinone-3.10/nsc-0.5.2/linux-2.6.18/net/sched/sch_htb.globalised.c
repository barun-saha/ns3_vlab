# 1 "linux-2.6.18/net/sched/sch_htb.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.18/net/sched/sch_htb.c"
# 30 "linux-2.6.18/net/sched/sch_htb.c"
...
# 31 "linux-2.6.18/net/sched/sch_htb.c" 2
# 1 "linux-2.6.18/include/asm/uaccess.h" 1
# 40 "linux-2.6.18/include/asm/uaccess.h"
typedef  struct movsl_mask { int  mask ;  }   _GLOBAL_118_T; extern  _GLOBAL_118_T  global_movsl_mask[NUM_STACKS];    
...
# 32 "linux-2.6.18/net/sched/sch_htb.c" 2
...
# 39 "linux-2.6.18/net/sched/sch_htb.c" 2
...
# 40 "linux-2.6.18/net/sched/sch_htb.c" 2
...
# 42 "linux-2.6.18/net/sched/sch_htb.c" 2
...
# 44 "linux-2.6.18/net/sched/sch_htb.c" 2
...
# 47 "linux-2.6.18/net/sched/sch_htb.c" 2
...
# 48 "linux-2.6.18/net/sched/sch_htb.c" 2
...
typedef  atomic_t  _GLOBAL_218_T; extern  _GLOBAL_218_T  global_flow_cache_genid[NUM_STACKS];   
...
# 50 "linux-2.6.18/net/sched/sch_htb.c" 2
...
# 51 "linux-2.6.18/net/sched/sch_htb.c" 2
...
# 56 "linux-2.6.18/net/sched/sch_htb.c" 2
# 130 "linux-2.6.18/net/sched/sch_htb.c"
enum htb_cmode {
    HTB_CANT_SEND,
    HTB_MAY_BORROW,
    HTB_CAN_SEND
...
struct htb_class
{
    ...
    union {
     ...
     struct htb_class_inner {
      struct rb_root feed[8];
      struct rb_node *ptr[8];
...
static __inline__  __attribute__((always_inline)) long L2T(struct htb_class *cl,struct qdisc_rate_table *rate,
 int size)
{
...
struct htb_sched
{
    ...
    struct list_head drops[8];


    struct rb_root row[8][8];
    int row_mask[8];
    struct rb_node *ptr[8][8];
    u32 last_ptr_id[8][8];


    struct rb_root wait_pq[8];


    unsigned long near_ev_cache[8];
...
static __inline__  __attribute__((always_inline)) int htb_hash(u32 h)
{
...
static __inline__  __attribute__((always_inline)) struct htb_class *htb_find(u32 handle, struct Qdisc *sch)
{
 struct htb_sched *q = qdisc_priv(sch);
 struct list_head *p;
 if (((handle)&(0xFFFF0000U)) != sch->handle)
  return ((void *)0);

 for (p = (q->hash+htb_hash(handle))->next; prefetch(p->next), p != (q->hash+htb_hash(handle)); p = p->next) {
  struct htb_class *cl = ({ const typeof( ((struct htb_class *)0)->hlist ) *__mptr = (p); (struct htb_class *)( (char *)__mptr - __builtin_offsetof(struct htb_class,hlist) );});
  if (cl->classid == handle)
   return cl;
 }
 return ((void *)0);
}
# 302 "linux-2.6.18/net/sched/sch_htb.c"
static inline  __attribute__((always_inline)) u32 htb_classid(struct htb_class *cl)
{
 return (cl && cl != (struct htb_class*)-1) ? cl->classid : (0U);
}
...
# 338 "linux-2.6.18/net/sched/sch_htb.c"
 ...
 cl = htb_find((((((sch->handle)&(0xFFFF0000U)))&(0xFFFF0000U))|((q->defcls)&(0x0000FFFFU))),sch);
 if (!cl || cl->level)
  return (struct htb_class*)-1;
 return cl;
}
# 409 "linux-2.6.18/net/sched/sch_htb.c"
static void htb_add_to_id_tree ( struct rb_root *root,
  struct htb_class *cl,int prio)
{
 struct rb_node **p = &root->rb_node, *parent = ((void *)0);
 ;
# 422 "linux-2.6.18/net/sched/sch_htb.c"
 while (*p) {
  struct htb_class *c; parent = *p;
  c = ({ const typeof( ((struct htb_class *)0)->node[prio] ) *__mptr = (parent); (struct htb_class *)( (char *)__mptr - __builtin_offsetof(struct htb_class,node[prio]) );});
  ;
...
# 442 "linux-2.6.18/net/sched/sch_htb.c"
static void htb_add_to_wait_tree (struct htb_sched *q,
  struct htb_class *cl,long delay,int debug_hint)
{
 struct rb_node **p = &q->wait_pq[cl->level].rb_node, *parent = ((void *)0);
 ;
 ...
 cl->pq_key = q->jiffies + usecs_to_jiffies(delay);
 if (cl->pq_key == q->jiffies)
  cl->pq_key++;


 if ((({ unsigned long __dummy; typeof(q->near_ev_cache[cl->level]) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ({ unsigned long __dummy; typeof(cl->pq_key) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ((long)(cl->pq_key) - (long)(q->near_ev_cache[cl->level]) < 0)))
  q->near_ev_cache[cl->level] = cl->pq_key;

 while (*p) {
  struct htb_class *c; parent = *p;
  c = ({ const typeof( ((struct htb_class *)0)->pq_node ) *__mptr = (parent); (struct htb_class *)( (char *)__mptr - __builtin_offsetof(struct htb_class,pq_node) );});
  if ((({ unsigned long __dummy; typeof(cl->pq_key) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ({ unsigned long __dummy; typeof(c->pq_key) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ((long)(cl->pq_key) - (long)(c->pq_key) >= 0)))
   p = &parent->rb_right;
  else
   p = &parent->rb_left;
 }
...
static inline  __attribute__((always_inline)) void htb_add_class_to_row(struct htb_sched *q,
...
static __inline__  __attribute__((always_inline)) void htb_remove_class_from_row(struct htb_sched *q,
  struct htb_class *cl,int mask)
{
 int m = 0;
 ;
...
# 536 "linux-2.6.18/net/sched/sch_htb.c"
...
# 573 "linux-2.6.18/net/sched/sch_htb.c"
...
# 620 "linux-2.6.18/net/sched/sch_htb.c"
static __inline__  __attribute__((always_inline)) enum htb_cmode
...
# 653 "linux-2.6.18/net/sched/sch_htb.c"
static void
...
# 681 "linux-2.6.18/net/sched/sch_htb.c"
static __inline__  __attribute__((always_inline)) void htb_activate(struct htb_sched *q,struct htb_class *cl)
{
 do { if (__builtin_expect(!!(!(!cl->level && cl->un.leaf.q && cl->un.leaf.q->q.qlen)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "!cl->level && cl->un.leaf.q && cl->un.leaf.q->q.qlen", "linux-2.6.18/net/sched/sch_htb.c" , 683); } } while(0);
 ;
 if (!cl->prio_activity) {
...
{
 do { if (__builtin_expect(!!(!(cl->prio_activity)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "cl->prio_activity", "linux-2.6.18/net/sched/sch_htb.c" , 701); } } while(0);
 ;
 htb_deactivate_prios(q,cl);
 cl->prio_activity = 0;
...
static int htb_enqueue(struct sk_buff *skb, struct Qdisc *sch)
{
 ...
 if (q->direct_queue.qlen < q->direct_qlen) {
    ...
    } else if (cl->un.leaf.q->enqueue(skb, cl->un.leaf.q) != 0) {
 sch->qstats.drops++;
 cl->qstats.drops++;
 return 1;
    } else {
...
static int htb_requeue(struct sk_buff *skb, struct Qdisc *sch)
{
 ...
 if (q->direct_queue.qlen < q->direct_qlen && cl) {
    ...
    } else if (cl->un.leaf.q->ops->requeue(skb, cl->un.leaf.q) != 0) {
 sch->qstats.drops++;
 cl->qstats.drops++;
 return 1;
    } else
...
static void htb_rate_timer(unsigned long arg)
{
 ...
 do { local_bh_disable(); do { do { } while (0); (void)0; (void)(&(sch)->dev->queue_lock); } while (0); } while (0);
 ;

 q->rttim.expires = global_jiffies[get_stack_id()] + 1000;
 add_timer(&q->rttim);


 if (++q->recmp_bucket >= 16)
  q->recmp_bucket = 0;
 for (p = (q->hash+q->recmp_bucket)->next; prefetch(p->next), p != (q->hash+q->recmp_bucket); p = p->next) {
  struct htb_class *cl = ({ const typeof( ((struct htb_class *)0)->hlist ) *__mptr = (p); (struct htb_class *)( (char *)__mptr - __builtin_offsetof(struct htb_class,hlist) );});
  ;

  cl->rate_bytes+=cl->sum_bytes-(cl->rate_bytes/2);cl->sum_bytes=0;
  cl->rate_packets+=cl->sum_packets-(cl->rate_packets/2);cl->sum_packets=0;
 }
 do { do { } while (0); local_bh_enable(); (void)0; (void)(&(sch)->dev->queue_lock); } while (0);
}
# 826 "linux-2.6.18/net/sched/sch_htb.c"
  ...
  int level,int bytes)
{
 ...
 while (cl) {
  ;
  diff = ({ int __delta_sec = (q->now).tv_sec - (cl->t_c).tv_sec; int __delta = (q->now).tv_usec - (cl->t_c).tv_usec; switch (__delta_sec) { default: __delta = psched_tod_diff(__delta_sec, (u32)cl->mbuffer); break; case 2: __delta += 1000000L; case 1: __delta += 1000000L; case 0: if (__delta > (u32)cl->mbuffer || __delta < 0) __delta = (u32)cl->mbuffer; } __delta; });
# 858 "linux-2.6.18/net/sched/sch_htb.c"
  if (cl->level >= level) {
   if (cl->level == level) cl->xstats.lends++;
   toks = diff + cl->tokens; if (toks > cl->buffer) toks = cl->buffer; toks -= L2T(cl, cl->rate, bytes); if (toks <= -cl->mbuffer) toks = 1-cl->mbuffer; cl->tokens = toks;
  } else {
   cl->xstats.borrows++;
   cl->tokens += diff;
  }
  toks = diff + cl->ctokens; if (toks > cl->cbuffer) toks = cl->cbuffer; toks -= L2T(cl, cl->ceil, bytes); if (toks <= -cl->mbuffer) toks = 1-cl->mbuffer; cl->ctokens = toks;
  cl->t_c = q->now;
  ;
...
# 899 "linux-2.6.18/net/sched/sch_htb.c"
static long htb_do_events(struct htb_sched *q,int level)
{
 ...
 for (i = 0; i < 500; i++) {
  ...
  cl = ({ const typeof( ((struct htb_class *)0)->pq_node ) *__mptr = (p); (struct htb_class *)( (char *)__mptr - __builtin_offsetof(struct htb_class,pq_node) );});
  if ((({ unsigned long __dummy; typeof(cl->pq_key) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ({ unsigned long __dummy; typeof(q->jiffies) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ((long)(q->jiffies) - (long)(cl->pq_key) < 0))) {
   ;
   return cl->pq_key - q->jiffies;
  }
  rb_erase(p,q->wait_pq+level);
  diff = ({ int __delta_sec = (q->now).tv_sec - (cl->t_c).tv_sec; int __delta = (q->now).tv_usec - (cl->t_c).tv_usec; switch (__delta_sec) { default: __delta = psched_tod_diff(__delta_sec, (u32)cl->mbuffer); break; case 2: __delta += 1000000L; case 1: __delta += 1000000L; case 0: if (__delta > (u32)cl->mbuffer || __delta < 0) __delta = (u32)cl->mbuffer; } __delta; });
# 934 "linux-2.6.18/net/sched/sch_htb.c"
  htb_change_class_mode(q,cl,&diff);
  if (cl->cmode != HTB_CAN_SEND)
   htb_add_to_wait_tree (q,cl,diff,2);
 }
 if (net_ratelimit())
  printk("<4>" "htb: too many events !\n");
 return 1000/10;
}
...
{
 struct rb_node *r = ((void *)0);
 while (n) {
  struct htb_class *cl = ({ const typeof( ((struct htb_class *)0)->node[prio] ) *__mptr = (n); (struct htb_class *)( (char *)__mptr - __builtin_offsetof(struct htb_class,node[prio]) );});
...
htb_lookup_leaf( struct rb_root *tree,int prio,struct rb_node **pptr,u32 *pid)
{
 ...
 do { if (__builtin_expect(!!(!(tree->rb_node)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "tree->rb_node", "linux-2.6.18/net/sched/sch_htb.c" , 978); } } while(0);
  ...
  *sp->pid = 0;
   ...
   *sp->pptr = sp->root;
   ...
   if (sp > stk) {
    sp--;
    do { if (__builtin_expect(!!(!(*sp->pptr)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "*sp->pptr", "linux-2.6.18/net/sched/sch_htb.c" , 999); } } while(0); if(!*sp->pptr) return ((void *)0);
    htb_next_rb_node (sp->pptr);
   }
  } else {
   struct htb_class *cl;
   cl = ({ const typeof( ((struct htb_class *)0)->node[prio] ) *__mptr = (*sp->pptr); (struct htb_class *)( (char *)__mptr - __builtin_offsetof(struct htb_class,node[prio]) );});
   ;
   if (!cl->level)
    return cl;
...
htb_dequeue_tree(struct htb_sched *q,int prio,int level)
{
 ...
 do {
next:
  do { if (__builtin_expect(!!(!(cl)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "cl", "linux-2.6.18/net/sched/sch_htb.c" , 1030); } } while(0);
  if (!cl) return ((void *)0);
  ;
  ...
  if (__builtin_expect(!!(cl->un.leaf.q->q.qlen == 0), 0)) {
  ...
  if (__builtin_expect(!!((skb = cl->un.leaf.q->dequeue(cl->un.leaf.q)) != ((void *)0)), 1))
   break;
  if (!cl->warned) {
   printk("<4>" "htb: class %X isn't work conserving ?!\n",cl->classid);
   cl->warned = 1;
  }
  q->nwc_hit++;
...
static void htb_delay_by(struct Qdisc *sch,long delay)
{
 struct htb_sched *q = qdisc_priv(sch);
 if (delay <= 0) delay = 1;
 if (__builtin_expect(!!(delay > 5*1000), 0)) {
  if (net_ratelimit())
   printk("<6>" "HTB delay %ld > 5sec\n", delay);
  delay = 5*1000;
 }
...
static struct sk_buff *htb_dequeue(struct Qdisc *sch)
{
 ...
 q->jiffies = global_jiffies[get_stack_id()];
 ;



 if ((skb = __skb_dequeue(&q->direct_queue)) != ((void *)0)) {
  sch->flags &= ~2;
  sch->q.qlen--;
  return skb;
 }
 ...
 do_gettimeofday(&(q->now));

 min_delay = ((long)(~0UL>>1));
 q->nwc_hit = 0;
 for (level = 0; level < 8; level++) {

  int m;
  long delay;
  if ((({ unsigned long __dummy; typeof(q->jiffies) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ({ unsigned long __dummy; typeof(q->near_ev_cache[level]) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ((long)(q->jiffies) - (long)(q->near_ev_cache[level]) >= 0))) {
   delay = htb_do_events(q,level);
   q->near_ev_cache[level] = q->jiffies + (delay ? delay : 1000);
...
# 1165 "linux-2.6.18/net/sched/sch_htb.c"
 htb_delay_by (sch,min_delay > 5*1000 ? 5*1000 : min_delay);
fin:
 ;
 return skb;
}
...
static unsigned int htb_drop(struct Qdisc* sch)
{
 ...
 for (prio = 8 - 1; prio >= 0; prio--) {
  struct list_head *p;
  for (p = (q->drops+prio)->next; prefetch(p->next), p != (q->drops+prio); p = p->next) {
   struct htb_class *cl = ({ const typeof( ((struct htb_class *)0)->un.leaf.drop_list ) *__mptr = (p); (struct htb_class *)( (char *)__mptr - __builtin_offsetof(struct htb_class,un.leaf.drop_list) );});
...
static void htb_reset(struct Qdisc* sch)
{
 ...
 for (i = 0; i < 16; i++) {
  struct list_head *p;
  for (p = (q->hash+i)->next; prefetch(p->next), p != (q->hash+i); p = p->next) {
   struct htb_class *cl = ({ const typeof( ((struct htb_class *)0)->hlist ) *__mptr = (p); (struct htb_class *)( (char *)__mptr - __builtin_offsetof(struct htb_class,hlist) );});
   if (cl->level)
    (__builtin_constant_p(0) ? (__builtin_constant_p((sizeof(cl->un.inner))) ? __constant_c_and_count_memset(((&cl->un.inner)),((0x01010101UL*(unsigned char)(0))),((sizeof(cl->un.inner)))) : __constant_c_memset(((&cl->un.inner)),((0x01010101UL*(unsigned char)(0))),((sizeof(cl->un.inner))))) : (__builtin_constant_p((sizeof(cl->un.inner))) ? __memset_generic((((&cl->un.inner))),(((0))),(((sizeof(cl->un.inner))))) : __memset_generic(((&cl->un.inner)),((0)),((sizeof(cl->un.inner))))));
   else {
    if (cl->un.leaf.q)
 ...
 sch->q.qlen = 0;
 (__builtin_constant_p(0) ? (__builtin_constant_p((sizeof(q->row))) ? __constant_c_and_count_memset(((q->row)),((0x01010101UL*(unsigned char)(0))),((sizeof(q->row)))) : __constant_c_memset(((q->row)),((0x01010101UL*(unsigned char)(0))),((sizeof(q->row))))) : (__builtin_constant_p((sizeof(q->row))) ? __memset_generic((((q->row))),(((0))),(((sizeof(q->row))))) : __memset_generic(((q->row)),((0)),((sizeof(q->row))))));
 (__builtin_constant_p(0) ? (__builtin_constant_p((sizeof(q->row_mask))) ? __constant_c_and_count_memset(((q->row_mask)),((0x01010101UL*(unsigned char)(0))),((sizeof(q->row_mask)))) : __constant_c_memset(((q->row_mask)),((0x01010101UL*(unsigned char)(0))),((sizeof(q->row_mask))))) : (__builtin_constant_p((sizeof(q->row_mask))) ? __memset_generic((((q->row_mask))),(((0))),(((sizeof(q->row_mask))))) : __memset_generic(((q->row_mask)),((0)),((sizeof(q->row_mask))))));
 (__builtin_constant_p(0) ? (__builtin_constant_p((sizeof(q->wait_pq))) ? __constant_c_and_count_memset(((q->wait_pq)),((0x01010101UL*(unsigned char)(0))),((sizeof(q->wait_pq)))) : __constant_c_memset(((q->wait_pq)),((0x01010101UL*(unsigned char)(0))),((sizeof(q->wait_pq))))) : (__builtin_constant_p((sizeof(q->wait_pq))) ? __memset_generic((((q->wait_pq))),(((0))),(((sizeof(q->wait_pq))))) : __memset_generic(((q->wait_pq)),((0)),((sizeof(q->wait_pq))))));
 (__builtin_constant_p(0) ? (__builtin_constant_p((sizeof(q->ptr))) ? __constant_c_and_count_memset(((q->ptr)),((0x01010101UL*(unsigned char)(0))),((sizeof(q->ptr)))) : __constant_c_memset(((q->ptr)),((0x01010101UL*(unsigned char)(0))),((sizeof(q->ptr))))) : (__builtin_constant_p((sizeof(q->ptr))) ? __memset_generic((((q->ptr))),(((0))),(((sizeof(q->ptr))))) : __memset_generic(((q->ptr)),((0)),((sizeof(q->ptr))))));
 for (i = 0; i < 8; i++)
...
static int htb_init(struct Qdisc *sch, struct rtattr *opt)
{
 ...
 if (!opt || rtattr_parse((tb), (TCA_HTB_INIT), ((void*)(((char*)((opt))) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))), ((int)(((opt))->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))) ||
   tb[TCA_HTB_INIT-1] == ((void *)0) ||
   ((int)((tb[TCA_HTB_INIT-1])->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) < sizeof(*gopt)) {
  printk("<3>" "HTB: hey probably you have bad tc tool ?\n");
  return -22;
 }
 gopt = ((void*)(((char*)(tb[TCA_HTB_INIT-1])) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))));
 if (gopt->version != 0x30011 >> 16) {
  printk("<3>" "HTB: need tc/htb version %d (minor is %d), you have %d\n",
    0x30011 >> 16,0x30011 & 0xffff,gopt->version);
  return -22;
 }
 q->debug = gopt->debug;
 ;
...
static int htb_dump(struct Qdisc *sch, struct sk_buff *skb)
{
 ...
 ({ if (__builtin_expect(!!(skb_tailroom(skb) < (int)( (((( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (sizeof(gopt))))+4 -1) & ~(4 -1) )), 0)) goto rtattr_failure; __rta_fill(skb, TCA_HTB_INIT, sizeof(gopt), &gopt); });
 ...
 struct sk_buff *skb, struct tcmsg *tcm)
{
 ...
 do { local_bh_disable(); do { do { } while (0); (void)0; (void)(&(sch)->dev->queue_lock); } while (0); } while (0);
 tcm->tcm_parent = cl->parent ? cl->parent->classid : (0xFFFFFFFFU);
 ...
 (__builtin_constant_p(0) ? (__builtin_constant_p((sizeof(opt))) ? __constant_c_and_count_memset(((&opt)),((0x01010101UL*(unsigned char)(0))),((sizeof(opt)))) : __constant_c_memset(((&opt)),((0x01010101UL*(unsigned char)(0))),((sizeof(opt))))) : (__builtin_constant_p((sizeof(opt))) ? __memset_generic((((&opt))),(((0))),(((sizeof(opt))))) : __memset_generic(((&opt)),((0)),((sizeof(opt))))));
 ...
 opt.level = cl->level;
 ({ if (__builtin_expect(!!(skb_tailroom(skb) < (int)( (((( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (sizeof(opt))))+4 -1) & ~(4 -1) )), 0)) goto rtattr_failure; __rta_fill(skb, TCA_HTB_PARMS, sizeof(opt), &opt); });
 rta->rta_len = skb->tail - b;
 do { do { } while (0); local_bh_enable(); (void)0; (void)(&(sch)->dev->queue_lock); } while (0);
 return skb->len;
rtattr_failure:
 ...
 struct gnet_dump *d)
{
 ...
 cl->rate_est.bps = cl->rate_bytes/(2*16);
 cl->rate_est.pps = cl->rate_packets/(2*16);
 ...
 struct Qdisc **old)
{
 ...
 if (cl && !cl->level) {
  if (new == ((void *)0) && (new = qdisc_create_dflt(sch->dev,
     &global_pfifo_qdisc_ops[get_stack_id()])) == ((void *)0))
     return -105;
  qdisc_lock_tree((sch)->dev);
  if ((*old = ((__typeof__(*(&cl->un.leaf.q)))__xchg((unsigned long)(new),(&cl->un.leaf.q),sizeof(*(&cl->un.leaf.q))))) != ((void *)0)) {
   if (cl->prio_activity)
...
{
 struct htb_class *cl = (struct htb_class*)arg;
 return (cl && !cl->level) ? cl->un.leaf.q : ((void *)0);
}
...
static void htb_destroy_class(struct Qdisc* sch,struct htb_class *cl)
{
 ...
 ;
 if (!cl->level) {
  do { if (__builtin_expect(!!(!(cl->un.leaf.q)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "cl->un.leaf.q", "linux-2.6.18/net/sched/sch_htb.c" , 1438); } } while(0);
 ...
 while (!list_empty(&cl->children))
  htb_destroy_class (sch,({ const typeof( ((struct htb_class *)0)->sibling ) *__mptr = (cl->children.next); (struct htb_class *)( (char *)__mptr - __builtin_offsetof(struct htb_class,sibling) );}));
...
static void htb_destroy(struct Qdisc* sch)
{
 ...
 while (!list_empty(&q->root))
  htb_destroy_class (sch,({ const typeof( ((struct htb_class *)0)->sibling ) *__mptr = (q->root.next); (struct htb_class *)( (char *)__mptr - __builtin_offsetof(struct htb_class,sibling) );}));
  ...
  u32 parentid, struct rtattr **tca, unsigned long *arg)
{
 ...
 struct qdisc_rate_table *rtab = ((void *)0), *ctab = ((void *)0);
 struct rtattr *tb[TCA_HTB_RTAB];
 struct tc_htb_opt *hopt;


 if (!opt || rtattr_parse((tb), (TCA_HTB_RTAB), ((void*)(((char*)((opt))) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))), ((int)(((opt))->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))) ||
   tb[TCA_HTB_PARMS-1] == ((void *)0) ||
   ((int)((tb[TCA_HTB_PARMS-1])->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) < sizeof(*hopt))
  goto failure;

 parent = parentid == (0xFFFFFFFFU) ? ((void *)0) : htb_find (parentid,sch);

 hopt = ((void*)(((char*)(tb[TCA_HTB_PARMS-1])) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))));
 ;
 ...
 if (!cl) {
  ...
  if (!classid || ((classid^sch->handle)&(0xFFFF0000U)) || htb_find(classid,sch))
   goto failure;


  if (parent && parent->parent && parent->parent->level < 2) {
   printk("<3>" "htb: tree is too deep\n");
   goto failure;
  }
  err = -105;
  if ((cl = kzalloc(sizeof(*cl), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)))) == ((void *)0))
   goto failure;
  ...
  new_q = qdisc_create_dflt(sch->dev, &global_pfifo_qdisc_ops[get_stack_id()]);
  qdisc_lock_tree((sch)->dev);
  if (parent && !parent->level) {
   ...
   parent->level = (parent->parent ? parent->parent->level
     : 8) - 1;
   (__builtin_constant_p(0) ? (__builtin_constant_p((sizeof(parent->un.inner))) ? __constant_c_and_count_memset(((&parent->un.inner)),((0x01010101UL*(unsigned char)(0))),((sizeof(parent->un.inner)))) : __constant_c_memset(((&parent->un.inner)),((0x01010101UL*(unsigned char)(0))),((sizeof(parent->un.inner))))) : (__builtin_constant_p((sizeof(parent->un.inner))) ? __memset_generic((((&parent->un.inner))),(((0))),(((sizeof(parent->un.inner))))) : __memset_generic(((&parent->un.inner)),((0)),((sizeof(parent->un.inner))))));
  }

  cl->un.leaf.q = new_q ? new_q : &global_noop_qdisc[get_stack_id()];
  ...
  cl->mbuffer = jiffies_to_usecs(1000*60);
  do_gettimeofday(&(cl->t_c));
  cl->cmode = HTB_CAN_SEND;
 ...
 if (!cl->level) {
  ...
  if (!hopt->quantum && cl->un.leaf.quantum < 1000) {
   printk("<4>" "HTB: quantum of class %X is small. Consider r2q change.\n", cl->classid);
   cl->un.leaf.quantum = 1000;
  }
  if (!hopt->quantum && cl->un.leaf.quantum > 200000) {
   printk("<4>" "HTB: quantum of class %X is big. Consider r2q change.\n", cl->classid);
   cl->un.leaf.quantum = 200000;
  }
  if (hopt->quantum)
   cl->un.leaf.quantum = hopt->quantum;
  if ((cl->un.leaf.prio = hopt->prio) >= 8)
   cl->un.leaf.prio = 8 - 1;
 }
 ...
 u32 classid)
{
 ...
 struct htb_class *cl = htb_find (classid,sch);
 ;
# 1676 "linux-2.6.18/net/sched/sch_htb.c"
 if (cl)
  cl->filter_cnt++;
 else
  q->filter_cnt++;
...
typedef  struct Qdisc_class_ops   _GLOBAL_266_T; static  _GLOBAL_266_T  global_htb_class_ops[NUM_STACKS] = {  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  { . graft  = htb_graft  , . leaf  = htb_leaf   , . get  = htb_get   , . put  = htb_put   , . change  = htb_change_class   , . delete  = htb_delete   , . walk  = htb_walk   , . tcf_chain  = htb_find_tcf   , . bind_tcf  = htb_bind_filter   , . unbind_tcf  = htb_unbind_filter   , . dump  = htb_dump_class   , . dump_stats  = htb_dump_class_stats   , } ,  };      
...
typedef  struct Qdisc_ops   _GLOBAL_267_T; static  _GLOBAL_267_T  global_htb_qdisc_ops[NUM_STACKS] = {  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[0]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[1]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[2]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[3]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[4]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[5]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[6]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[7]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[8]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[9]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[10]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[11]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[12]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[13]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[14]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[15]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[16]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[17]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[18]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[19]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[20]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[21]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[22]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[23]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[24]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[25]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[26]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[27]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[28]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[29]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[30]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[31]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[32]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[33]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[34]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[35]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[36]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[37]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[38]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[39]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[40]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[41]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[42]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[43]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[44]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[45]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[46]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[47]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[48]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_htb_class_ops[49]    , . id  = "htb"   , . priv_size  = sizeof ( struct htb_sched  )    , . enqueue  = htb_enqueue   , . dequeue  = htb_dequeue   , . requeue  = htb_requeue   , . drop  = htb_drop   , . init  = htb_init   , . reset  = htb_reset   , . destroy  = htb_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = htb_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  };      
...
static int  __attribute__ ((__section__ (".init.text"))) htb_module_init(void)
{
    return register_qdisc(&global_htb_qdisc_ops[get_stack_id()]);
}
static void   __attribute__((__used__)) __attribute__ ((__section__(".exit.text"))) htb_module_exit(void)
{
    unregister_qdisc(&global_htb_qdisc_ops[get_stack_id()]);
}
typedef  initcall_t  _GLOBAL_268_T; static  _GLOBAL_268_T  __initcall_htb_module_init __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init")))  = htb_module_init ;       
typedef  exitcall_t  _GLOBAL_269_T; static  _GLOBAL_269_T  global___exitcall_htb_module_exit[NUM_STACKS] __attribute__((__used__)) __attribute__ ((__section__ (".exitcall.exit")))   = {  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  htb_module_exit,  };       
;
