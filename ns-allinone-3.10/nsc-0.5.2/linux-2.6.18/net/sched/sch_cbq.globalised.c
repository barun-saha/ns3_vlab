# 1 "linux-2.6.18/net/sched/sch_cbq.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.18/net/sched/sch_cbq.c"
# 13 "linux-2.6.18/net/sched/sch_cbq.c"
...
# 14 "linux-2.6.18/net/sched/sch_cbq.c" 2
...
# 15 "linux-2.6.18/net/sched/sch_cbq.c" 2
...
# 22 "linux-2.6.18/net/sched/sch_cbq.c" 2
...
# 23 "linux-2.6.18/net/sched/sch_cbq.c" 2
...
# 25 "linux-2.6.18/net/sched/sch_cbq.c" 2
...
# 27 "linux-2.6.18/net/sched/sch_cbq.c" 2
...
# 30 "linux-2.6.18/net/sched/sch_cbq.c" 2
...
# 31 "linux-2.6.18/net/sched/sch_cbq.c" 2
...
# 33 "linux-2.6.18/net/sched/sch_cbq.c" 2
...
# 34 "linux-2.6.18/net/sched/sch_cbq.c" 2
...
# 37 "linux-2.6.18/net/sched/sch_cbq.c" 2
# 87 "linux-2.6.18/net/sched/sch_cbq.c"
...
struct cbq_class
{
 ...
 struct cbq_class *defaults[15 +1];
};

struct cbq_sched_data
{
 struct cbq_class *classes[16];
 int nclasses[8 +1];
 unsigned quanta[8 +1];
...
static __inline__  __attribute__((always_inline)) unsigned cbq_hash(u32 h)
{
 h ^= h>>8;
 h ^= h>>4;
 return h&0xF;
...
static __inline__  __attribute__((always_inline)) struct cbq_class *
...
# 242 "linux-2.6.18/net/sched/sch_cbq.c"
...
cbq_classify(struct sk_buff *skb, struct Qdisc *sch, int *qerr)
{
 ...
 if (((prio^sch->handle)&(0xFFFF0000U)) == 0 &&
 ...
 *qerr = 4;
 for (;;) {
  ...
  if ((cl = (void*)res.class) == ((void *)0)) {
   if (((res.classid)&(0xFFFF0000U)))
    cl = cbq_class_lookup(q, res.classid);
   else if ((cl = defmap[res.classid&15]) == ((void *)0))
    cl = defmap[0];

   if (cl == ((void *)0) || cl->level >= head->level)
    goto fallback;
  }
# 298 "linux-2.6.18/net/sched/sch_cbq.c"
  if (cl->level == 0)
   return cl;
...
fallback:
 ...
 if (((prio)&(0xFFFF0000U)) == 0 &&
     !(cl = head->defaults[prio&15]) &&
     !(cl = head->defaults[0]))
  return head;
...
static __inline__  __attribute__((always_inline)) void cbq_activate_class(struct cbq_class *cl)
{
...
cbq_mark_toplevel(struct cbq_sched_data *q, struct cbq_class *cl)
{
 ...
 if (toplevel > cl->level && !(cl->q->flags&2)) {
  psched_time_t now;
  psched_tdiff_t incr;

  do_gettimeofday(&(now));
  incr = ({ int __delta_sec = (now).tv_sec - (q->now_rt).tv_sec; int __delta = (now).tv_usec - (q->now_rt).tv_usec; if (__delta_sec) { switch (__delta_sec) { default: __delta = 0; case 2: __delta += 1000000L; case 1: __delta += 1000000L; } } __delta; });
  ({ int __delta = (q->now).tv_usec + (incr); (now).tv_sec = (q->now).tv_sec; while (__delta >= 1000000L) { (now).tv_sec++; __delta -= 1000000L; } (now).tv_usec = __delta; });

  do {
   if ((((cl->undertime).tv_usec < (now).tv_usec && (cl->undertime).tv_sec <= (now).tv_sec) || (cl->undertime).tv_sec < (now).tv_sec)) {
    q->toplevel = cl->level;
    return;
   }
...
{
 struct cbq_sched_data *q = qdisc_priv(cl->qdisc);
 psched_tdiff_t delay = ({ int __delta_sec = (cl->undertime).tv_sec - (q->now).tv_sec; int __delta = (cl->undertime).tv_usec - (q->now).tv_usec; if (__delta_sec) { switch (__delta_sec) { default: __delta = 0; case 2: __delta += 1000000L; case 1: __delta += 1000000L; } } __delta; });

 if (!cl->delayed) {
  delay += cl->offtime;
# 491 "linux-2.6.18/net/sched/sch_cbq.c"
  if (cl->avgidle < 0)
  ...
  if (delay <= 0)
   delay = 1;
  ({ int __delta = (q->now).tv_usec + (delay); (cl->undertime).tv_sec = (q->now).tv_sec; while (__delta >= 1000000L) { (cl->undertime).tv_sec++; __delta -= 1000000L; } (cl->undertime).tv_usec = __delta; });
 ...
 if (q->toplevel == 8) {
  ...
  for (b = cl->borrow; b; b = b->borrow) {
   delay = ({ int __delta_sec = (b->undertime).tv_sec - (q->now).tv_sec; int __delta = (b->undertime).tv_usec - (q->now).tv_usec; if (__delta_sec) { switch (__delta_sec) { default: __delta = 0; case 2: __delta += 1000000L; case 1: __delta += 1000000L; } } __delta; });
   if (delay < base_delay) {
    if (delay <= 0)
     delay = 1;
    base_delay = delay;
...
static void cbq_ovl_delay(struct cbq_class *cl)
{
 ...
 if (!cl->delayed) {
  unsigned long sched = global_jiffies[get_stack_id()];
  ...
  if (delay > 0) {
   sched += usecs_to_jiffies(delay) + cl->penalty;
   cl->penalized = sched;
   cl->cpriority = 8;
   q->pmask |= (1<<8);
...
static void cbq_ovl_lowprio(struct cbq_class *cl)
{
 ...
 cl->penalized = global_jiffies[get_stack_id()] + cl->penalty;
...
static void cbq_undelay(unsigned long arg)
{
 ...
 if (delay) {
  q->delay_timer.expires = global_jiffies[get_stack_id()] + delay;
  add_timer(&q->delay_timer);
 }
...
# 731 "linux-2.6.18/net/sched/sch_cbq.c"
static __inline__  __attribute__((always_inline)) void
      ...
      struct cbq_class *borrowed)
{
 if (cl && q->toplevel >= borrowed->level) {
  if (cl->q->q.qlen > 1) {
   do {
    if (((borrowed->undertime).tv_sec == 0)) {
...
cbq_update(struct cbq_sched_data *q)
{
 ...
 for ( ; cl; cl = cl->share) {
  ...
  cl->bstats.packets++;
  cl->bstats.bytes += len;
# 776 "linux-2.6.18/net/sched/sch_cbq.c"
  idle = ({ int __delta_sec = (q->now).tv_sec - (cl->last).tv_sec; int __delta = (q->now).tv_usec - (cl->last).tv_usec; if (__delta_sec) { switch (__delta_sec) { default: __delta = 0; case 2: __delta += 1000000L; case 1: __delta += 1000000L; } } __delta; });
  if ((unsigned long)idle > 128*1024*1024) {
   avgidle = cl->maxidle;
  } else {
   idle -= ((cl)->R_tab->data[(len)>>(cl)->R_tab->rate.cell_log]);
  ...
  if (avgidle <= 0) {
   ...
   cl->avgidle = avgidle;
# 806 "linux-2.6.18/net/sched/sch_cbq.c"
   idle = (-avgidle) - ((-avgidle) >> cl->ewma_log);
# 817 "linux-2.6.18/net/sched/sch_cbq.c"
   idle -= ((&q->link)->R_tab->data[(len)>>(&q->link)->R_tab->rate.cell_log]);
   idle += ((cl)->R_tab->data[(len)>>(cl)->R_tab->rate.cell_log]);

   ({ if ((idle) > 2000000) (idle) = 2000000; });

   ({ int __delta = (q->now).tv_usec + (idle); (cl->undertime).tv_sec = (q->now).tv_sec; while (__delta >= 1000000L) { (cl->undertime).tv_sec++; __delta -= 1000000L; } (cl->undertime).tv_usec = __delta; });
  } else {
...
cbq_under_limit(struct cbq_class *cl)
{
 ...
 if (((cl->undertime).tv_sec == 0) ||
     !(((q->now).tv_usec < (cl->undertime).tv_usec && (q->now).tv_sec <= (cl->undertime).tv_sec) || (q->now).tv_sec < (cl->undertime).tv_sec)) {
  cl->delayed = 0;
  return cl;
 }

 do {
# 864 "linux-2.6.18/net/sched/sch_cbq.c"
...
static __inline__  __attribute__((always_inline)) struct sk_buff *
...
cbq_dequeue(struct Qdisc *sch)
{
 ...
 if (q->tx_class) {
  ...
  incr2 = ((&q->link)->R_tab->data[(q->tx_len)>>(&q->link)->R_tab->rate.cell_log]);
  ({ (q->now).tv_usec += (incr2); while ((q->now).tv_usec >= 1000000L) { (q->now).tv_sec++; (q->now).tv_usec -= 1000000L; } });
  cbq_update(q);
  if ((incr -= incr2) < 0)
   incr = 0;
 }
 ({ (q->now).tv_usec += (incr); while ((q->now).tv_usec >= 1000000L) { (q->now).tv_sec++; (q->now).tv_usec -= 1000000L; } });
 q->now_rt = now;
...
# 1056 "linux-2.6.18/net/sched/sch_cbq.c"
  if (q->toplevel == 8 &&
      ((q->link.undertime).tv_sec == 0))
   break;
 ...
 if (sch->q.qlen) {
  ...
  if (q->wd_expires) {
   long delay = usecs_to_jiffies(q->wd_expires);
   if (delay <= 0)
    delay = 1;
 ...
 for (h=0; h<16; h++) {
  for (cl = q->classes[h]; cl; cl = cl->next) {
   ...
   if (cl->quantum <= 0 || cl->quantum>32*cl->qdisc->dev->mtu) {
    printk("<4>" "CBQ: class %08x has bad quantum==%ld, repaired.\n", cl->classid, cl->quantum);
...
static void cbq_change_defmap(struct cbq_class *cl, u32 splitid, u32 def, u32 mask)
{
 ...
 if (split == ((void *)0) || split->classid != splitid) {
...
static void cbq_unlink_class(struct cbq_class *this)
{
 ...
 for (clp = &q->classes[cbq_hash(this->classid)]; (cl = *clp) != ((void *)0); clp = &cl->next) {
  if (cl == this) {
   *clp = cl->next;
   cl->next = ((void *)0);
   break;
 ...
 if (this->tparent) {
  ...
  do { if (__builtin_expect(!!(!(this->sibling == this)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "this->sibling == this", "linux-2.6.18/net/sched/sch_cbq.c" , 1221); } } while(0);
 }
}
...
{
 if (lss->change&1) {
  cl->share = (lss->flags&2) ? ((void *)0) : cl->tparent;
  cl->borrow = (lss->flags&1) ? ((void *)0) : cl->tparent;
 }
 if (lss->change&2)
...
static int cbq_set_overlimit(struct cbq_class *cl, struct tc_cbq_ovl *ovl)
{
 ...
 cl->penalty = (ovl->penalty*1000)/1000;
 return 0;
}
# 1402 "linux-2.6.18/net/sched/sch_cbq.c"
...
{
 struct cbq_sched_data *q = qdisc_priv(sch);
 struct rtattr *tb[(__TCA_CBQ_MAX - 1)];
 struct tc_ratespec *r;

 if (rtattr_parse((tb), ((__TCA_CBQ_MAX - 1)), ((void*)(((char*)((opt))) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))), ((int)(((opt))->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))) < 0 ||
     tb[TCA_CBQ_RTAB-1] == ((void *)0) || tb[TCA_CBQ_RATE-1] == ((void *)0) ||
     ((int)((tb[TCA_CBQ_RATE-1])->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) < sizeof(struct tc_ratespec))
  return -22;

 if (tb[TCA_CBQ_LSSOPT-1] &&
     ((int)((tb[TCA_CBQ_LSSOPT-1])->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) < sizeof(struct tc_cbq_lssopt))
  return -22;

 r = ((void*)(((char*)(tb[TCA_CBQ_RATE-1])) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))));

 if ((q->link.R_tab = qdisc_get_rtab(r, tb[TCA_CBQ_RTAB-1])) == ((void *)0))
  return -22;
 ...
 q->link.qdisc = sch;
 if (!(q->link.q = qdisc_create_dflt(sch->dev, &global_pfifo_qdisc_ops[get_stack_id()])))
  q->link.q = &global_noop_qdisc[get_stack_id()];

 q->link.priority = 8 -1;
 q->link.priority2 = 8 -1;
 q->link.cpriority = 8 -1;
 q->link.ovl_strategy = 0;
 ...
 if (tb[TCA_CBQ_LSSOPT-1])
  cbq_set_lss(&q->link, ((void*)(((char*)(tb[TCA_CBQ_LSSOPT-1])) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))));

 cbq_addprio(q, &q->link);
 return 0;
}

static __inline__  __attribute__((always_inline)) int cbq_dump_rate(struct sk_buff *skb, struct cbq_class *cl)
{
 unsigned char *b = skb->tail;

 ({ if (__builtin_expect(!!(skb_tailroom(skb) < (int)( (((( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (sizeof(cl->R_tab->rate))))+4 -1) & ~(4 -1) )), 0)) goto rtattr_failure; __rta_fill(skb, TCA_CBQ_RATE, sizeof(cl->R_tab->rate), &cl->R_tab->rate); });
 return skb->len;
...
static __inline__  __attribute__((always_inline)) int cbq_dump_lss(struct sk_buff *skb, struct cbq_class *cl)
{
 unsigned char *b = skb->tail;
 ...
 opt.change = ~0;
 ({ if (__builtin_expect(!!(skb_tailroom(skb) < (int)( (((( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (sizeof(opt))))+4 -1) & ~(4 -1) )), 0)) goto rtattr_failure; __rta_fill(skb, TCA_CBQ_LSSOPT, sizeof(opt), &opt); });
 return skb->len;
...
static __inline__  __attribute__((always_inline)) int cbq_dump_wrr(struct sk_buff *skb, struct cbq_class *cl)
{
 unsigned char *b = skb->tail;
 ...
 ({ if (__builtin_expect(!!(skb_tailroom(skb) < (int)( (((( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (sizeof(opt))))+4 -1) & ~(4 -1) )), 0)) goto rtattr_failure; __rta_fill(skb, TCA_CBQ_WRROPT, sizeof(opt), &opt); });
 return skb->len;
...
static __inline__  __attribute__((always_inline)) int cbq_dump_ovl(struct sk_buff *skb, struct cbq_class *cl)
{
 unsigned char *b = skb->tail;
 ...
 opt.pad = 0;
 opt.penalty = (cl->penalty*1000)/1000;
 ({ if (__builtin_expect(!!(skb_tailroom(skb) < (int)( (((( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (sizeof(opt))))+4 -1) & ~(4 -1) )), 0)) goto rtattr_failure; __rta_fill(skb, TCA_CBQ_OVL_STRATEGY, sizeof(opt), &opt); });
 return skb->len;
...
static __inline__  __attribute__((always_inline)) int cbq_dump_fopt(struct sk_buff *skb, struct cbq_class *cl)
{
 unsigned char *b = skb->tail;
 ...
 if (cl->split || cl->defmap) {
  ...
  opt.defchange = ~0;
  ({ if (__builtin_expect(!!(skb_tailroom(skb) < (int)( (((( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (sizeof(opt))))+4 -1) & ~(4 -1) )), 0)) goto rtattr_failure; __rta_fill(skb, TCA_CBQ_FOPT, sizeof(opt), &opt); });
 }
 return skb->len;
...
# 1578 "linux-2.6.18/net/sched/sch_cbq.c"
 ...
 struct gnet_dump *d)
{
 ...
 if (!((cl->undertime).tv_sec == 0))
  cl->xstats.undertime = ({ int __delta_sec = (cl->undertime).tv_sec - (q->now).tv_sec; int __delta = (cl->undertime).tv_usec - (q->now).tv_usec; if (__delta_sec) { switch (__delta_sec) { default: __delta = 0; case 2: __delta += 1000000L; case 1: __delta += 1000000L; } } __delta; });
       ...
       struct Qdisc **old)
{
 ...
 if (cl) {
  if (new == ((void *)0)) {
   if ((new = qdisc_create_dflt(sch->dev, &global_pfifo_qdisc_ops[get_stack_id()])) == ((void *)0))
    return -105;
  } else {
...
static void cbq_destroy_filters(struct cbq_class *cl)
{
 ...
 while ((tp = cl->filter_list) != ((void *)0)) {
...
static void cbq_destroy_class(struct Qdisc *sch, struct cbq_class *cl)
{
 ...
 do { if (__builtin_expect(!!(!(!cl->filters)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "!cl->filters", "linux-2.6.18/net/sched/sch_cbq.c" , 1731); } } while(0);
...
cbq_destroy(struct Qdisc* sch)
{
 ...
 struct cbq_class *cl;
 unsigned h;
# 1758 "linux-2.6.18/net/sched/sch_cbq.c"
 for (h = 0; h < 16; h++)
  for (cl = q->classes[h]; cl; cl = cl->next)
   cbq_destroy_filters(cl);
...
static void cbq_put(struct Qdisc *sch, unsigned long arg)
{
 ...
 if (--cl->refcnt == 0) {
# 1786 "linux-2.6.18/net/sched/sch_cbq.c"
  cbq_destroy_class(sch, cl);
 }
}
   ...
   unsigned long *arg)
{
 ...
 if (opt==((void *)0) || rtattr_parse((tb), ((__TCA_CBQ_MAX - 1)), ((void*)(((char*)((opt))) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))), ((int)(((opt))->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))))
  return -22;

 if (tb[TCA_CBQ_OVL_STRATEGY-1] &&
     ((int)((tb[TCA_CBQ_OVL_STRATEGY-1])->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) < sizeof(struct tc_cbq_ovl))
  return -22;

 if (tb[TCA_CBQ_FOPT-1] &&
     ((int)((tb[TCA_CBQ_FOPT-1])->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) < sizeof(struct tc_cbq_fopt))
  return -22;
 ...
 if (tb[TCA_CBQ_WRROPT-1] &&
     ((int)((tb[TCA_CBQ_WRROPT-1])->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) < sizeof(struct tc_cbq_wrropt))
   return -22;
 ...
 if (cl) {
  ...
  if (parentid) {
   if (cl->tparent && cl->tparent->classid != parentid)
    return -22;
   if (!cl->tparent && parentid != (0xFFFFFFFFU))
    return -22;
  }

  if (tb[TCA_CBQ_RATE-1]) {
   rtab = qdisc_get_rtab(((void*)(((char*)(tb[TCA_CBQ_RATE-1])) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))), tb[TCA_CBQ_RTAB-1]);
   if (rtab == ((void *)0))
    return -22;
  }
  ...
  if (rtab) {
   rtab = ((__typeof__(*(&cl->R_tab)))__xchg((unsigned long)(rtab),(&cl->R_tab),sizeof(*(&cl->R_tab))));
   qdisc_put_rtab(rtab);
  }

  if (tb[TCA_CBQ_LSSOPT-1])
   cbq_set_lss(cl, ((void*)(((char*)(tb[TCA_CBQ_LSSOPT-1])) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))));

  if (tb[TCA_CBQ_WRROPT-1]) {
   cbq_rmprio(q, cl);
   cbq_set_wrr(cl, ((void*)(((char*)(tb[TCA_CBQ_WRROPT-1])) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))));
  }

  if (tb[TCA_CBQ_OVL_STRATEGY-1])
   cbq_set_overlimit(cl, ((void*)(((char*)(tb[TCA_CBQ_OVL_STRATEGY-1])) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))));
  ...
  if (tb[TCA_CBQ_FOPT-1])
   cbq_set_fopt(cl, ((void*)(((char*)(tb[TCA_CBQ_FOPT-1])) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))));
 ...
 if (tb[TCA_CBQ_WRROPT-1] == ((void *)0) || tb[TCA_CBQ_RATE-1] == ((void *)0) ||
     tb[TCA_CBQ_LSSOPT-1] == ((void *)0))
  return -22;
 ...
 if (classid) {
  err = -22;
  if (((classid^sch->handle)&(0xFFFF0000U)) || cbq_class_lookup(q, classid))
   goto failure;
 } else {
  int i;
  classid = (((sch->handle)&(0xFFFF0000U))|((0x8000)&(0x0000FFFFU)));
 ...
 err = -105;
 cl = kzalloc(sizeof(*cl), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (cl == ((void *)0))
  goto failure;
 cl->R_tab = rtab;
 rtab = ((void *)0);
 cl->refcnt = 1;
 if (!(cl->q = qdisc_create_dflt(sch->dev, &global_pfifo_qdisc_ops[get_stack_id()])))
  cl->q = &global_noop_qdisc[get_stack_id()];
 cl->classid = classid;
 cl->tparent = parent;
 cl->qdisc = sch;
...
typedef  struct Qdisc_class_ops   _GLOBAL_266_T; static  _GLOBAL_266_T  global_cbq_class_ops[NUM_STACKS] = {  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  { . graft  = cbq_graft  , . leaf  = cbq_leaf   , . get  = cbq_get   , . put  = cbq_put   , . change  = cbq_change_class   , . delete  = cbq_delete   , . walk  = cbq_walk   , . tcf_chain  = cbq_find_tcf   , . bind_tcf  = cbq_bind_filter   , . unbind_tcf  = cbq_unbind_filter   , . dump  = cbq_dump_class   , . dump_stats  = cbq_dump_class_stats   , } ,  };      
...
typedef  struct Qdisc_ops   _GLOBAL_267_T; static  _GLOBAL_267_T  global_cbq_qdisc_ops[NUM_STACKS] = {  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[0]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[1]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[2]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[3]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[4]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[5]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[6]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[7]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[8]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[9]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[10]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[11]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[12]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[13]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[14]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[15]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[16]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[17]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[18]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[19]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[20]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[21]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[22]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[23]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[24]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[25]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[26]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[27]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[28]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[29]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[30]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[31]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[32]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[33]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[34]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[35]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[36]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[37]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[38]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[39]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[40]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[41]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[42]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[43]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[44]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[45]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[46]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[47]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[48]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_cbq_class_ops[49]    , . id  = "cbq"   , . priv_size  = sizeof ( struct cbq_sched_data  )    , . enqueue  = cbq_enqueue   , . dequeue  = cbq_dequeue   , . requeue  = cbq_requeue   , . drop  = cbq_drop   , . init  = cbq_init   , . reset  = cbq_reset   , . destroy  = cbq_destroy   , . change  = ( ( void *  ) 0  )    , . dump  = cbq_dump   , . dump_stats  = cbq_dump_stats   , . owner  = ( ( struct module  *  ) 0  )    , } ,  };      
...
static int  __attribute__ ((__section__ (".init.text"))) cbq_module_init(void)
{
 return register_qdisc(&global_cbq_qdisc_ops[get_stack_id()]);
}
static void   __attribute__((__used__)) __attribute__ ((__section__(".exit.text"))) cbq_module_exit(void)
{
 unregister_qdisc(&global_cbq_qdisc_ops[get_stack_id()]);
}
typedef  initcall_t  _GLOBAL_268_T; static  _GLOBAL_268_T  __initcall_cbq_module_init __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init")))  = cbq_module_init ;       
typedef  exitcall_t  _GLOBAL_269_T; static  _GLOBAL_269_T  global___exitcall_cbq_module_exit[NUM_STACKS] __attribute__((__used__)) __attribute__ ((__section__ (".exitcall.exit")))   = {  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  cbq_module_exit,  };       
;
