...
append_opt:
  ({ if (__builtin_expect(!!(skb_tailroom(skb) < (int)(sizeof(opt))), 0)) goto rtattr_failure; ({ size_t __len = (sizeof(opt)); void *__ret; if (__builtin_constant_p(sizeof(opt)) && __len >= 64) __ret = __memcpy((skb_put(skb, sizeof(opt))),(&opt),__len); else __ret = __builtin_memcpy((skb_put(skb, sizeof(opt))),(&opt),__len); __ret; }); });
 }
