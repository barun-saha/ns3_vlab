# 1 "linux-2.6.18/net/sched/sch_netem.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.18/net/sched/sch_netem.c"
# 16 "linux-2.6.18/net/sched/sch_netem.c"
...
# 17 "linux-2.6.18/net/sched/sch_netem.c" 2
...
# 22 "linux-2.6.18/net/sched/sch_netem.c" 2
...
# 24 "linux-2.6.18/net/sched/sch_netem.c" 2
...
# 26 "linux-2.6.18/net/sched/sch_netem.c" 2
# 55 "linux-2.6.18/net/sched/sch_netem.c"
...
static int netem_enqueue(struct sk_buff *skb, struct Qdisc *sch)
{
 ...
 if (q->gap == 0
  ...
  do_gettimeofday(&(now));
  ({ int __delta = (now).tv_usec + (delay); (cb->time_to_send).tv_sec = (now).tv_sec; while (__delta >= 1000000L) { (cb->time_to_send).tv_sec++; __delta -= 1000000L; } (cb->time_to_send).tv_usec = __delta; });
  ++q->counter;
  ...
  do_gettimeofday(&(cb->time_to_send));
  q->counter = 0;
...
static struct sk_buff *netem_dequeue(struct Qdisc *sch)
{
 ...
 if (skb) {
  ...
  if ((((cb->time_to_send).tv_usec < (now).tv_usec && (cb->time_to_send).tv_sec <= (now).tv_sec) || (cb->time_to_send).tv_sec < (now).tv_sec)) {
   do { } while (0);
   sch->q.qlen--;
   sch->flags &= ~2;
   return skb;
  } else {
   psched_tdiff_t delay = ({ int __delta_sec = (cb->time_to_send).tv_sec - (now).tv_sec; int __delta = (cb->time_to_send).tv_usec - (now).tv_usec; if (__delta_sec) { switch (__delta_sec) { default: __delta = 0; case 2: __delta += 1000000L; case 1: __delta += 1000000L; } } __delta; });

   if (q->qdisc->ops->requeue(skb, q->qdisc) != 0) {
    sch->qstats.drops++;


    printk("<3>" "netem: queue discpline %s could not requeue\n",
           q->qdisc->ops->id);

    sch->q.qlen--;
   }

   mod_timer(&q->timer, global_jiffies[get_stack_id()] + usecs_to_jiffies(delay));
   sch->flags |= 2;
  }
 }
...
{
 struct netem_sched_data *q = qdisc_priv(sch);
 unsigned long n = ((int)((attr)->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))/sizeof(__s16);
 const __s16 *data = ((void*)(((char*)(attr)) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))));
 struct disttable *d;
 int i;
...
{
 struct netem_sched_data *q = qdisc_priv(sch);
 const struct tc_netem_corr *c = ((void*)(((char*)(attr)) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))));

 if (((int)((attr)->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) != sizeof(*c))
  return -22;
...
{
 struct netem_sched_data *q = qdisc_priv(sch);
 const struct tc_netem_reorder *r = ((void*)(((char*)(attr)) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))));

 if (((int)((attr)->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) != sizeof(*r))
  return -22;
...
{
 struct netem_sched_data *q = qdisc_priv(sch);
 const struct tc_netem_corrupt *r = ((void*)(((char*)(attr)) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))));
...
static int netem_change(struct Qdisc *sch, struct rtattr *opt)
{
 ...
 if (((int)((opt)->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) > sizeof(*qopt)) {
  struct rtattr *tb[(__TCA_NETEM_MAX - 1)];
  if (rtattr_parse(tb, (__TCA_NETEM_MAX - 1),
     ((void*)(((char*)(opt)) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))) + sizeof(*qopt),
     ((int)((opt)->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) - sizeof(*qopt)))
   return -22;
...
static int tfifo_enqueue(struct sk_buff *nskb, struct Qdisc *sch)
{
 ...
 if (__builtin_expect(!!(skb_queue_len(list) < q->limit), 1)) {
  for (skb = (list)->prev; prefetch(skb->prev), (skb != (struct sk_buff *)(list)); skb = skb->prev) {
   ...
   if (!(((ncb->time_to_send).tv_usec < (cb->time_to_send).tv_usec && (ncb->time_to_send).tv_sec <= (cb->time_to_send).tv_sec) || (ncb->time_to_send).tv_sec < (cb->time_to_send).tv_sec))
    break;
  }
...
static int tfifo_init(struct Qdisc *sch, struct rtattr *opt)
{
 ...
 if (opt) {
  struct tc_fifo_qopt *ctl = ((void*)(((char*)(opt)) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))));
  if (((int)((opt)->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) < sizeof(*ctl))
   return -22;

  q->limit = ctl->limit;
 } else
  q->limit = ({ u32 __x = (sch->dev->tx_queue_len); u32 __y = (1); __x > __y ? __x: __y; });
...
static int netem_dump(struct Qdisc *sch, struct sk_buff *skb)
{
 ...
 ({ if (__builtin_expect(!!(skb_tailroom(skb) < (int)( (((( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (sizeof(cor))))+4 -1) & ~(4 -1) )), 0)) goto rtattr_failure; __rta_fill(skb, TCA_NETEM_CORR, sizeof(cor), &cor); });

 reorder.probability = q->reorder;
 reorder.correlation = q->reorder_cor.rho;
 ({ if (__builtin_expect(!!(skb_tailroom(skb) < (int)( (((( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (sizeof(reorder))))+4 -1) & ~(4 -1) )), 0)) goto rtattr_failure; __rta_fill(skb, TCA_NETEM_REORDER, sizeof(reorder), &reorder); });

 corrupt.probability = q->corrupt;
 corrupt.correlation = q->corrupt_cor.rho;
 ({ if (__builtin_expect(!!(skb_tailroom(skb) < (int)( (((( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (sizeof(corrupt))))+4 -1) & ~(4 -1) )), 0)) goto rtattr_failure; __rta_fill(skb, TCA_NETEM_CORRUPT, sizeof(corrupt), &corrupt); });
