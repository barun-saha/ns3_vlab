# 1 "linux-2.6.18/net/sched/sch_prio.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.18/net/sched/sch_prio.c"
# 14 "linux-2.6.18/net/sched/sch_prio.c"
...
# 15 "linux-2.6.18/net/sched/sch_prio.c" 2
...
# 16 "linux-2.6.18/net/sched/sch_prio.c" 2
...
# 23 "linux-2.6.18/net/sched/sch_prio.c" 2
...
# 24 "linux-2.6.18/net/sched/sch_prio.c" 2
...
# 26 "linux-2.6.18/net/sched/sch_prio.c" 2
...
# 28 "linux-2.6.18/net/sched/sch_prio.c" 2
...
# 31 "linux-2.6.18/net/sched/sch_prio.c" 2
...
# 32 "linux-2.6.18/net/sched/sch_prio.c" 2
...
# 34 "linux-2.6.18/net/sched/sch_prio.c" 2
...
# 35 "linux-2.6.18/net/sched/sch_prio.c" 2
...
# 38 "linux-2.6.18/net/sched/sch_prio.c" 2
 ...
 *qerr = 4;
 if (((skb->priority)&(0xFFFF0000U)) != sch->handle) {
# 69 "linux-2.6.18/net/sched/sch_prio.c"
  if (!q->filter_list || tc_classify(skb, q->filter_list, &res)) {

   if (((band)&(0xFFFF0000U)))
    band = 0;
   return q->queues[q->prio2band[band&15]];
  }
  band = res.classid;
 }
 band = ((band)&(0x0000FFFFU)) - 1;
 if (band > q->bands)
...
# 101 "linux-2.6.18/net/sched/sch_prio.c"
...
prio_requeue(struct sk_buff *skb, struct Qdisc* sch)
{
 ...
 qdisc = prio_classify(skb, sch, &ret);
# 128 "linux-2.6.18/net/sched/sch_prio.c"
...
{
 struct prio_sched_data *q = qdisc_priv(sch);
 struct tc_prio_qopt *qopt = ((void*)(((char*)(opt)) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))));
 int i;

 if (opt->rta_len < (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (sizeof(*qopt))))
  return -22;
 if (qopt->bands > 16 || qopt->bands < 2)
  return -22;
 ...
 q->bands = qopt->bands;
 (__builtin_constant_p(15 +1) ? __constant_memcpy((q->prio2band),(qopt->priomap),(15 +1)) : __memcpy((q->prio2band),(qopt->priomap),(15 +1)));

 for (i=q->bands; i<16; i++) {
  struct Qdisc *child = ((__typeof__(*(&q->queues[i])))__xchg((unsigned long)(&global_noop_qdisc[get_stack_id()]),(&q->queues[i]),sizeof(*(&q->queues[i]))));
  if (child != &global_noop_qdisc[get_stack_id()])
   qdisc_destroy(child);
 }
 qdisc_unlock_tree((sch)->dev);

 for (i=0; i<q->bands; i++) {
  if (q->queues[i] == &global_noop_qdisc[get_stack_id()]) {
   struct Qdisc *child;
   child = qdisc_create_dflt(sch->dev, &global_pfifo_qdisc_ops[get_stack_id()]);
   if (child) {
    qdisc_lock_tree((sch)->dev);
    child = ((__typeof__(*(&q->queues[i])))__xchg((unsigned long)(child),(&q->queues[i]),sizeof(*(&q->queues[i]))));
...
static int prio_init(struct Qdisc *sch, struct rtattr *opt)
{
 ...
 for (i=0; i<16; i++)
  q->queues[i] = &global_noop_qdisc[get_stack_id()];
...
static int prio_dump(struct Qdisc *sch, struct sk_buff *skb)
{
 ...
 opt.bands = q->bands;
 (__builtin_constant_p(15 +1) ? __constant_memcpy((&opt.priomap),(q->prio2band),(15 +1)) : __memcpy((&opt.priomap),(q->prio2band),(15 +1)));
...
{
 struct prio_sched_data *q = qdisc_priv(sch);
 unsigned long band = ((classid)&(0x0000FFFFU));
...
typedef  struct Qdisc_class_ops   _GLOBAL_266_T; static  _GLOBAL_266_T  global_prio_class_ops[NUM_STACKS] = {  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  { . graft  = prio_graft  , . leaf  = prio_leaf   , . get  = prio_get   , . put  = prio_put   , . change  = prio_change   , . delete  = prio_delete   , . walk  = prio_walk   , . tcf_chain  = prio_find_tcf   , . bind_tcf  = prio_bind   , . unbind_tcf  = prio_put   , . dump  = prio_dump_class   , } ,  };      
...
typedef  struct Qdisc_ops   _GLOBAL_267_T; static  _GLOBAL_267_T  global_prio_qdisc_ops[NUM_STACKS] = {  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[0]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[1]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[2]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[3]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[4]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[5]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[6]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[7]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[8]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[9]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[10]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[11]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[12]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[13]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[14]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[15]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[16]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[17]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[18]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[19]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[20]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[21]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[22]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[23]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[24]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[25]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[26]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[27]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[28]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[29]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[30]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[31]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[32]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[33]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[34]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[35]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[36]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[37]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[38]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[39]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[40]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[41]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[42]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[43]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[44]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[45]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[46]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[47]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[48]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[49]    , . id  = "prio"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = prio_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  };      
...
static int  __attribute__ ((__section__ (".init.text"))) prio_module_init(void)
{
 return register_qdisc(&global_prio_qdisc_ops[get_stack_id()]);
}

static void   __attribute__((__used__)) __attribute__ ((__section__(".exit.text"))) prio_module_exit(void)
{
 unregister_qdisc(&global_prio_qdisc_ops[get_stack_id()]);
}

typedef  initcall_t  _GLOBAL_268_T; static  _GLOBAL_268_T  __initcall_prio_module_init __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init")))  = prio_module_init ;       
typedef  exitcall_t  _GLOBAL_269_T; static  _GLOBAL_269_T  global___exitcall_prio_module_exit[NUM_STACKS] __attribute__((__used__)) __attribute__ ((__section__ (".exitcall.exit")))   = {  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  };       
