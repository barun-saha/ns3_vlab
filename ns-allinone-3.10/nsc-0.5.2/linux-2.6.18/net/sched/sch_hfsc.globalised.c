# 1 "linux-2.6.18/net/sched/sch_hfsc.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.18/net/sched/sch_hfsc.c"
# 52 "linux-2.6.18/net/sched/sch_hfsc.c"
...
# 53 "linux-2.6.18/net/sched/sch_hfsc.c" 2
...
# 54 "linux-2.6.18/net/sched/sch_hfsc.c" 2
...
# 60 "linux-2.6.18/net/sched/sch_hfsc.c" 2
...
# 67 "linux-2.6.18/net/sched/sch_hfsc.c" 2
...
# 68 "linux-2.6.18/net/sched/sch_hfsc.c" 2
...
# 69 "linux-2.6.18/net/sched/sch_hfsc.c" 2
...
# 10 "linux-2.6.18/include/net/sch_generic.h"
...
# 70 "linux-2.6.18/net/sched/sch_hfsc.c" 2
...
# 71 "linux-2.6.18/net/sched/sch_hfsc.c" 2
# 89 "linux-2.6.18/net/sched/sch_hfsc.c"
struct internal_sc
{
 u64 sm1;
 u64 ism1;
...
# 233 "linux-2.6.18/net/sched/sch_hfsc.c"
static void
...
# 439 "linux-2.6.18/net/sched/sch_hfsc.c"
...
m2sm(u32 m)
{
 ...
 sm = ((u64)m << 20);
 sm += jiffies_to_usecs(1000) - 1;
 ({ unsigned long __upper, __low, __high, __mod, __base; __base = (jiffies_to_usecs(1000)); asm("":"=a" (__low), "=d" (__high):"A" (sm)); __upper = __high; if (__high) { __upper = __high % (__base); __high = __high / (__base); } asm("divl %2":"=a" (__low), "=d" (__mod):"rm" (__base), "0" (__low), "1" (__upper)); asm("":"=A" (sm):"a" (__low),"d" (__high)); __mod; });
 return sm;
}
...
m2ism(u32 m)
{
 ...
 if (m == 0)
  ...
  ism = ((u64)jiffies_to_usecs(1000) << 18);
  ism += m - 1;
...
d2dx(u32 d)
{
 ...
 dx = ((u64)d * jiffies_to_usecs(1000));
 dx += 1000000L - 1;
...
dx2d(u64 dx)
{
 ...
 d = dx * 1000000L;
 ({ unsigned long __upper, __low, __high, __mod, __base; __base = (jiffies_to_usecs(1000)); asm("":"=a" (__low), "=d" (__high):"A" (d)); __upper = __high; if (__high) { __upper = __high % (__base); __high = __high / (__base); } asm("divl %2":"=a" (__low), "=d" (__mod):"rm" (__base), "0" (__low), "1" (__upper)); asm("":"=A" (d):"a" (__low),"d" (__high)); __mod; });
 return (u32)d;
}
...
init_ed(struct hfsc_class *cl, unsigned int next_len)
{
 ...
 do { struct timeval tv; do_gettimeofday(&tv); (cur_time) = 1ULL * 1000000L * tv.tv_sec + tv.tv_usec; } while (0);
...
init_vf(struct hfsc_class *cl, unsigned int len)
{
 ...
 for (; cl->cl_parent != ((void *)0); cl = cl->cl_parent) {
  ...
  f = ({ typeof(cl->cl_myf) _x = (cl->cl_myf); typeof(cl->cl_cfmin) _y = (cl->cl_cfmin); (void) (&_x == &_y); _x > _y ? _x : _y; });
  if (f != cl->cl_f) {
   cl->cl_f = f;
   cftree_update(cl);
 ...
 for (; cl->cl_parent != ((void *)0); cl = cl->cl_parent) {
  ...
  if (cl->cl_flags & HFSC_USC) {
   cl->cl_myf = cl->cl_myfadj + rtsc_y2x(&cl->cl_ulimit,
                                         cl->cl_total);
# 894 "linux-2.6.18/net/sched/sch_hfsc.c"
  }
                  ...
                  struct rtattr **tca, unsigned long *arg)
{
 ...
 struct rtattr *tb[(__TCA_HFSC_MAX - 1)];
 struct tc_service_curve *rsc = ((void *)0), *fsc = ((void *)0), *usc = ((void *)0);
 u64 cur_time;

 if (opt == ((void *)0) || rtattr_parse((tb), ((__TCA_HFSC_MAX - 1)), ((void*)(((char*)((opt))) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))), ((int)(((opt))->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0)))))
  return -22;

 if (tb[TCA_HFSC_RSC-1]) {
  if (((int)((tb[TCA_HFSC_RSC-1])->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) < sizeof(*rsc))
   return -22;
  rsc = ((void*)(((char*)(tb[TCA_HFSC_RSC-1])) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))));
  if (rsc->m1 == 0 && rsc->m2 == 0)
   rsc = ((void *)0);
 }

 if (tb[TCA_HFSC_FSC-1]) {
  if (((int)((tb[TCA_HFSC_FSC-1])->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) < sizeof(*fsc))
   return -22;
  fsc = ((void*)(((char*)(tb[TCA_HFSC_FSC-1])) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))));
  if (fsc->m1 == 0 && fsc->m2 == 0)
   fsc = ((void *)0);
 }

 if (tb[TCA_HFSC_USC-1]) {
  if (((int)((tb[TCA_HFSC_USC-1])->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) < sizeof(*usc))
   return -22;
  usc = ((void*)(((char*)(tb[TCA_HFSC_USC-1])) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))));
 ...
 cl->sched = q;
 cl->cl_parent = parent;
 cl->qdisc = qdisc_create_dflt(sch->dev, &global_pfifo_qdisc_ops[get_stack_id()]);
 if (cl->qdisc == ((void *)0))
...
# 1243 "linux-2.6.18/net/sched/sch_hfsc.c"
...
hfsc_dump_sc(struct sk_buff *skb, int attr, struct internal_sc *sc)
{
 ...
 tsc.d = dx2d(sc->dx);
 tsc.m2 = sm2m(sc->sm2);
 ({ if (__builtin_expect(!!(skb_tailroom(skb) < (int)( (((( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (sizeof(tsc))))+4 -1) & ~(4 -1) )), 0)) goto rtattr_failure; __rta_fill(skb, attr, sizeof(tsc), &tsc); });
...
hfsc_schedule_watchdog(struct Qdisc *sch, u64 cur_time)
{
 ...
 do { if (__builtin_expect(!!(!(next_time != 0)), 0)) printk("assertion %s failed at %s:%i (%s)\n", "next_time != 0", "linux-2.6.18/net/sched/sch_hfsc.c", 1486, (__func__)); } while (0);
...
hfsc_init_qdisc(struct Qdisc *sch, struct rtattr *opt)
{
 ...
 if (opt == ((void *)0) || ((int)((opt)->rta_len) - (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))) < sizeof(*qopt))
  return -22;
 qopt = ((void*)(((char*)(opt)) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))));
...
hfsc_dump_qdisc(struct Qdisc *sch, struct sk_buff *skb)
{
 ...
 ({ if (__builtin_expect(!!(skb_tailroom(skb) < (int)( (((( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (sizeof(qopt))))+4 -1) & ~(4 -1) )), 0)) goto rtattr_failure; __rta_fill(skb, TCA_OPTIONS, sizeof(qopt), &qopt); });
 return skb->len;
