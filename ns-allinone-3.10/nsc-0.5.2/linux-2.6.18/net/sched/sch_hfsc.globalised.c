...
# 1 "linux-2.6.18/include/asm/div64.h" 1
# 73 "linux-2.6.18/net/sched/sch_hfsc.c" 2
...
m2sm(u32 m)
{
 ...
 ({ uint32_t __base = (jiffies_to_usecs(1000)); uint32_t __rem; __rem = ((uint64_t)(sm)) % __base; (sm) = ((uint64_t)(sm)) / __base; __rem; });
 return sm;
}
...
m2ism(u32 m)
{
 ...
 if (m == 0)
  ...
  ism += m - 1;
  ({ uint32_t __base = (m); uint32_t __rem; __rem = ((uint64_t)(ism)) % __base; (ism) = ((uint64_t)(ism)) / __base; __rem; });
 }
 return ism;
}
...
d2dx(u32 d)
{
 ...
 dx += 1000000L - 1;
 ({ uint32_t __base = (1000000L); uint32_t __rem; __rem = ((uint64_t)(dx)) % __base; (dx) = ((uint64_t)(dx)) / __base; __rem; });
 return dx;
}
...
dx2d(u64 dx)
{
 ...
 d = dx * 1000000L;
 ({ uint32_t __base = (jiffies_to_usecs(1000)); uint32_t __rem; __rem = ((uint64_t)(d)) % __base; (d) = ((uint64_t)(d)) / __base; __rem; });
 return (u32)d;
}
...
rtsc_min(struct runtime_sc *rtsc, struct internal_sc *isc, u64 x, u64 y)
{
 ...
 dx = (y1 - y) << 20;
 dsm = isc->sm1 - isc->sm2;
 ({ uint32_t __base = (dsm); uint32_t __rem; __rem = ((uint64_t)(dx)) % __base; (dx) = ((uint64_t)(dx)) / __base; __rem; });
