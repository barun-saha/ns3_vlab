# 1 "linux-2.6.18/net/core/datagram.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.18/net/core/datagram.c"
# 36 "linux-2.6.18/net/core/datagram.c"
...
# 37 "linux-2.6.18/net/core/datagram.c" 2
...
# 40 "linux-2.6.18/net/core/datagram.c" 2
...
# 42 "linux-2.6.18/net/core/datagram.c" 2
...
typedef  int  _GLOBAL_197_T; extern  _GLOBAL_197_T  global_sysctl_somaxconn[NUM_STACKS];   
...
# 47 "linux-2.6.18/net/core/datagram.c" 2
...
# 48 "linux-2.6.18/net/core/datagram.c" 2
...
# 53 "linux-2.6.18/net/core/datagram.c" 2
# 62 "linux-2.6.18/net/core/datagram.c"
static inline  __attribute__((always_inline)) int connection_based(struct sock *sk)
{
...
static int wait_for_packet(struct sock *sk, int *err, long *timeo_p)
{
 ...
 prepare_to_wait_exclusive(sk->sk_sleep, &wait, 1);
 ...
 if (connection_based(sk) &&
     !(sk->__sk_common.skc_state == TCP_ESTABLISHED || sk->__sk_common.skc_state == TCP_LISTEN))
  goto out_err;
...
# 146 "linux-2.6.18/net/core/datagram.c"
      ...
      int noblock, int *err)
{
 ...
 do {
  ...
  if (flags & 2) {
   ...
   do { (void)(cpu_flags); do { do { } while (0); (void)0; (void)(&sk->sk_receive_queue.lock); } while (0); } while (0);
...
# 222 "linux-2.6.18/net/core/datagram.c"
...
;
# 247 "linux-2.6.18/net/core/datagram.c"
       ...
       struct iovec *to, int len)
{
 ...
 if (((struct skb_shared_info *)((skb)->end))->frag_list) {
  ...
  for (; list; list = list->next) {
   ...
   do { if (__builtin_expect(!!(!(start <= offset + len)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "start <= offset + len", "linux-2.6.18/net/core/datagram.c" , 298); } } while(0);
          ...
          unsigned int *csump)
{
 ...
 for (i = 0; i < ((struct skb_shared_info *)((skb)->end))->nr_frags; i++) {
  ...
  do { if (__builtin_expect(!!(!(start <= offset + len)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "start <= offset + len", "linux-2.6.18/net/core/datagram.c" , 349); } } while(0);
 ...
 if (((struct skb_shared_info *)((skb)->end))->frag_list) {
  ...
  for (; list; list=list->next) {
   ...
   do { if (__builtin_expect(!!(!(start <= offset + len)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "start <= offset + len", "linux-2.6.18/net/core/datagram.c" , 385); } } while(0);
...
;
# 441 "linux-2.6.18/net/core/datagram.c"
...
# 491 "linux-2.6.18/net/core/datagram.c"
