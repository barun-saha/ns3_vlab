...
int ethtool_op_get_perm_addr(struct net_device *dev, struct ethtool_perm_addr *addr, u8 *data)
{
 ...
 addr->size = len;
 ({ size_t __len = (len); void *__ret; if (__builtin_constant_p(len) && __len >= 64) __ret = __memcpy((data),(dev->perm_addr),__len); else __ret = __builtin_memcpy((data),(dev->perm_addr),__len); __ret; });
 return 0;
}
