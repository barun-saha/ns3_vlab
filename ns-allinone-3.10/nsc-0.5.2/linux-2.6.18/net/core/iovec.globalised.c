# 1 "linux-2.6.18/net/core/iovec.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.18/net/core/iovec.c"
# 19 "linux-2.6.18/net/core/iovec.c"
...
# 20 "linux-2.6.18/net/core/iovec.c" 2
...
# 21 "linux-2.6.18/net/core/iovec.c" 2
...
# 24 "linux-2.6.18/net/core/iovec.c" 2
...
# 26 "linux-2.6.18/net/core/iovec.c" 2
...
typedef  const struct in6_addr    _GLOBAL_176_T; extern  _GLOBAL_176_T  global_in6addr_loopback[NUM_STACKS];     
...
# 27 "linux-2.6.18/net/core/iovec.c" 2
...
# 28 "linux-2.6.18/net/core/iovec.c" 2
...
# 30 "linux-2.6.18/net/core/iovec.c" 2
...
# 31 "linux-2.6.18/net/core/iovec.c" 2
# 40 "linux-2.6.18/net/core/iovec.c"
...
int memcpy_toiovec(struct iovec *iov, unsigned char *kdata, int len)
{
 while (len > 0) {
  if (iov->iov_len) {
   int copy = ({ unsigned int __x = (iov->iov_len); unsigned int __y = (len); __x < __y ? __x: __y; });
...
int memcpy_fromiovec(unsigned char *kdata, struct iovec *iov, int len)
{
 while (len > 0) {
  if (iov->iov_len) {
   int copy = ({ unsigned int __x = (len); unsigned int __y = (iov->iov_len); __x < __y ? __x: __y; });
   ...
   int len)
{
 ...
 while (len > 0) {
  ...
  int copy = ({ unsigned int __x = (len); unsigned int __y = (iov->iov_len - offset); __x < __y ? __x: __y; });
...
# 160 "linux-2.6.18/net/core/iovec.c"
