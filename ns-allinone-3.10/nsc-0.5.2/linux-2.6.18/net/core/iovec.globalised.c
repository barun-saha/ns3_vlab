     ...
     int offset, unsigned int len, int *csump)
{
 ...
 while (len > 0) {
  ...
  if (copy) {
   csum = csum_partial_copy_from_user(base, kdata, copy,
       csum, &err);
   if (err)
    goto out;
  }
