...
# 1 "<command-line>" 2
# 1 "linux-2.6.18/net/core/link_watch.c"
# 14 "linux-2.6.18/net/core/link_watch.c"
...
# 15 "linux-2.6.18/net/core/link_watch.c" 2
...
# 16 "linux-2.6.18/net/core/link_watch.c" 2
...
# 19 "linux-2.6.18/net/core/link_watch.c" 2
# 29 "linux-2.6.18/net/core/link_watch.c"
enum lw_bits {
 LW_RUNNING = 0,
 LW_SE_USED
};
...
void linkwatch_run_queue(void)
{
 ...
 list_replace_init(&global_lweventlist[get_stack_id()], &head);
 do { ; do { do { } while (0); (void)0; (void)(&global_lweventlist_lock[get_stack_id()]); } while (0); } while (0);

 for (n = (&head)->next, next = n->next; n != (&head); n = next, next = n->next) {
  struct lw_event *event = ({ const typeof( ((struct lw_event *)0)->list ) *__mptr = (n); (struct lw_event *)( (char *)__mptr - __builtin_offsetof(struct lw_event,list) );});
  struct net_device *dev = event->dev;

  if (event == &global_singleevent[get_stack_id()]) {
   clear_bit(LW_SE_USED, &global_linkwatch_flags[get_stack_id()]);
  } else {
   kfree(event);
  }
  ...
  if (dev->flags & 0x1) {
   if (netif_carrier_ok(dev)) {
    do { if (dev->qdisc_sleeping == &global_noop_qdisc[get_stack_id()]) ; } while(0);
    dev_activate(dev);
   } else
    dev_deactivate(dev);
...
static void linkwatch_event(void *dummy)
{
 ...
 clear_bit(LW_RUNNING, &global_linkwatch_flags[get_stack_id()]);
...
void linkwatch_fire_event(struct net_device *dev)
{
 if (!test_and_set_bit(__LINK_STATE_LINKWATCH_PENDING, &dev->state)) {
  ...
  if (test_and_set_bit(LW_SE_USED, &global_linkwatch_flags[get_stack_id()])) {
   event = kmalloc(sizeof(struct lw_event), ((( gfp_t)0x20u)));

   if (__builtin_expect(!!(event == ((void *)0)), 0)) {
    clear_bit(__LINK_STATE_LINKWATCH_PENDING, &dev->state);
    return;
   }
  } else {
   event = &global_singleevent[get_stack_id()];
  }
  ...
  list_add_tail(&event->list, &global_lweventlist[get_stack_id()]);
  do { (void)(flags); do { do { } while (0); (void)0; (void)(&global_lweventlist_lock[get_stack_id()]); } while (0); } while (0);

  if (!test_and_set_bit(LW_RUNNING, &global_linkwatch_flags[get_stack_id()])) {
   unsigned long delay = global_linkwatch_nextevent[get_stack_id()] - global_jiffies[get_stack_id()];


   if (!delay || delay > 1000)
    schedule_work(&global_linkwatch_work[get_stack_id()]);
   else
    schedule_delayed_work(&global_linkwatch_work[get_stack_id()], delay);
  }
 }
}
