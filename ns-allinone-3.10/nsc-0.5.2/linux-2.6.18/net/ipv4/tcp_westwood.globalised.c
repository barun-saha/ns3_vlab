# 1 "linux-2.6.18/net/ipv4/tcp_westwood.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.18/net/ipv4/tcp_westwood.c"
# 24 "linux-2.6.18/net/ipv4/tcp_westwood.c"
...
# 25 "linux-2.6.18/net/ipv4/tcp_westwood.c" 2
...
# 26 "linux-2.6.18/net/ipv4/tcp_westwood.c" 2
...
# 27 "linux-2.6.18/net/ipv4/tcp_westwood.c" 2
...
# 28 "linux-2.6.18/net/ipv4/tcp_westwood.c" 2
...
# 29 "linux-2.6.18/net/ipv4/tcp_westwood.c" 2
...
# 61 "linux-2.6.18/net/ipv4/tcp_westwood.c"
...
# 138 "linux-2.6.18/net/ipv4/tcp_westwood.c"
 if (w->rtt && delta > ({ u32 __x = (w->rtt); u32 __y = ((1000/20)); __x > __y ? __x: __y; })) {
...
static inline  __attribute__((always_inline)) void update_rtt_min(struct westwood *w)
{
 if (w->reset_rtt_min) {
  ...
  w->rtt_min = ({ typeof(w->rtt) _x = (w->rtt); typeof(w->rtt_min) _y = (w->rtt_min); (void) (&_x == &_y); _x < _y ? _x : _y; });
}
# 162 "linux-2.6.18/net/ipv4/tcp_westwood.c"
...
# 217 "linux-2.6.18/net/ipv4/tcp_westwood.c"
static u32 tcp_westwood_bw_rttmin(const struct sock *sk)
{
 const struct tcp_sock *tp = tcp_sk(sk);
 const struct westwood *w = inet_csk_ca(sk);
 return ({ u32 __x = ((w->bw_est * w->rtt_min) / tp->mss_cache); u32 __y = (2); __x > __y ? __x: __y; });
}
         ...
         struct sk_buff *skb)
{
 ...
 if (ext & (1 << (INET_DIAG_VEGASINFO - 1))) {
  ...
  rta = ({ if (__builtin_expect(!!(skb_tailroom(skb) < (int)( (((( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (sizeof(*info))))+4 -1) & ~(4 -1) )), 0)) goto rtattr_failure; __rta_reserve(skb, INET_DIAG_VEGASINFO, sizeof(*info)); });
  info = ((void*)(((char*)(rta)) + (( ((sizeof(struct rtattr))+4 -1) & ~(4 -1) ) + (0))));
  info->tcpv_enabled = 1;
  info->tcpv_rttcnt = 0;
