# 1 "linux-2.6.18/net/ipv4/inet_timewait_sock.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.18/net/ipv4/inet_timewait_sock.c"
# 12 "linux-2.6.18/net/ipv4/inet_timewait_sock.c"
# 1 "linux-2.6.18/include/net/inet_hashtables.h" 1
# 18 "linux-2.6.18/include/net/inet_hashtables.h"
...
# 6 "linux-2.6.18/include/linux/interrupt.h" 2
...
# 9 "linux-2.6.18/include/linux/interrupt.h" 2
...
# 7 "linux-2.6.18/include/linux/hardirq.h" 2
...
# 12 "linux-2.6.18/include/linux/irq.h"
...
# 13 "linux-2.6.18/include/linux/irq.h" 2
...
# 54 "linux-2.6.18/include/linux/spinlock.h"
...
# 17 "linux-2.6.18/include/linux/spinlock_types.h"
...
# 19 "linux-2.6.18/include/linux/irq.h" 2
...
# 19 "linux-2.6.18/include/net/inet_hashtables.h" 2
...
typedef  const struct in6_addr    _GLOBAL_132_T; extern  _GLOBAL_132_T  global_in6addr_loopback[NUM_STACKS];     
...
typedef  int  _GLOBAL_133_T; extern  _GLOBAL_133_T  global_sysctl_somaxconn[NUM_STACKS];   
...
extern int inet_hash_connect(struct inet_timewait_death_row *death_row,
        struct sock *sk);
# 13 "linux-2.6.18/net/ipv4/inet_timewait_sock.c" 2
...
# 15 "linux-2.6.18/net/ipv4/inet_timewait_sock.c" 2
...
void __inet_twsk_kill(struct inet_timewait_sock *tw, struct inet_hashinfo *hashinfo)
{
 ...
 struct inet_ehash_bucket *ehead = inet_ehash_bucket(hashinfo, tw->__tw_common.skc_hash);

 do { do { } while (0); (void)0; (void)(&ehead->lock); } while (0);
 if (hlist_unhashed(&tw->__tw_common.skc_node)) {
  do { do { } while (0); (void)0; (void)(&ehead->lock); } while (0);
  return;
 }
 __hlist_del(&tw->__tw_common.skc_node);
 sk_node_init(&tw->__tw_common.skc_node);
 do { do { } while (0); (void)0; (void)(&ehead->lock); } while (0);


 bhead = &hashinfo->bhash[inet_bhashfn(tw->tw_num, hashinfo->bhash_size)];
 do { do { } while (0); (void)0; (void)(&bhead->lock); } while (0);
 tb = tw->tw_tb;
 __hlist_del(&tw->__tw_common.skc_bind_node);
 tw->tw_tb = ((void *)0);
      ...
      struct inet_hashinfo *hashinfo)
{
 ...
 struct inet_ehash_bucket *ehead = inet_ehash_bucket(hashinfo, sk->__sk_common.skc_hash);
 struct inet_bind_hashbucket *bhead;
 ...
 do { if (__builtin_expect(!!(!(icsk->icsk_bind_hash)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "icsk->icsk_bind_hash", "linux-2.6.18/net/ipv4/inet_timewait_sock.c" , 71); } } while(0);
 ...
 atomic_inc(&tw->__tw_common.skc_refcnt);
...
struct inet_timewait_sock *inet_twsk_alloc(const struct sock *sk, const int state)
{
 ...
 if (tw != ((void *)0)) {
  ...
  tw->__tw_common.skc_bound_dev_if = sk->__sk_common.skc_bound_dev_if;
  tw->tw_num = inet->num;
  tw->__tw_common.skc_state = TCP_TIME_WAIT;
  tw->tw_substate = state;
  tw->tw_sport = inet->sport;
  tw->tw_dport = inet->dport;
  tw->__tw_common.skc_family = sk->__sk_common.skc_family;
  tw->__tw_common.skc_reuse = sk->__sk_common.skc_reuse;
  tw->__tw_common.skc_hash = sk->__sk_common.skc_hash;
  tw->tw_ipv6only = 0;
  tw->__tw_common.skc_prot = sk->sk_prot_creator;
  (((&tw->__tw_common.skc_refcnt)->counter) = (1));
  inet_twsk_dead_node_init(tw);
  __module_get(tw->__tw_common.skc_prot->owner);
 }
        ...
        const int slot)
{
 ...
 ret = 0;
rescan:
 for (node = (&twdr->cells[slot])->first; node && ({ prefetch(node->next); 1;}) && ({ tw = ({ const typeof( ((typeof(*tw) *)0)->tw_death_node ) *__mptr = (node); (typeof(*tw) *)( (char *)__mptr - __builtin_offsetof(typeof(*tw),tw_death_node) );}); 1;}); node = node->next) {
  __inet_twsk_del_dead_node(tw);
  do { do { } while (0); (void)0; (void)(&twdr->death_lock); } while (0);
 ...
 (({ (void)(0); ((*_GLOBAL_net_statistics_59_A[get_stack_id()])[0]); })->mibs[LINUX_MIB_TIMEWAITED] += killed);
...
void inet_twdr_hangman(unsigned long data)
{
 ...
 twdr->slot = ((twdr->slot + 1) & (8 - 1));
 if (need_timer)
  mod_timer(&twdr->tw_timer, global_jiffies[get_stack_id()] + twdr->period);
out:
 do { do { } while (0); (void)0; (void)(&twdr->death_lock); } while (0);
}
...
void inet_twdr_twkill_work(void *data)
{
 ...
 if ((8 - 1) > (sizeof(twdr->thread_slots) * 8))
  twkill_slots_invalid();

 while (twdr->thread_slots) {
  do { local_bh_disable(); do { do { } while (0); (void)0; (void)(&twdr->death_lock); } while (0); } while (0);
  for (i = 0; i < 8; i++) {
   ...
   while (inet_twdr_do_twkill_work(twdr, i) != 0) {
    if (need_resched()) {
     do { do { } while (0); local_bh_enable(); (void)0; (void)(&twdr->death_lock); } while (0);
     schedule();
     do { local_bh_disable(); do { do { } while (0); (void)0; (void)(&twdr->death_lock); } while (0); } while (0);
    }
   }
         ...
         const int timeo, const int timewait_len)
{
 struct hlist_head *list;
 int slot;
# 281 "linux-2.6.18/net/ipv4/inet_timewait_sock.c"
 slot = (timeo + (1 << (10 + 2 - 5)) - 1) >> (10 + 2 - 5);
 ...
 if (slot >= (1 << 5)) {
  ...
  tw->tw_ttd = global_jiffies[get_stack_id()] + timeo;
  slot = (twdr->slot + slot) & (8 - 1);
  list = &twdr->cells[slot];
 } else {
  tw->tw_ttd = global_jiffies[get_stack_id()] + (slot << (10 + 2 - 5));

  if (twdr->twcal_hand < 0) {
   twdr->twcal_hand = 0;
   twdr->twcal_jiffie = global_jiffies[get_stack_id()];
   twdr->twcal_timer.expires = twdr->twcal_jiffie +
           (slot << (10 + 2 - 5));
   add_timer(&twdr->twcal_timer);
  } else {
   if ((({ unsigned long __dummy; typeof(twdr->twcal_timer.expires) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ({ unsigned long __dummy; typeof(global_jiffies[get_stack_id()] + (slot << (10 + 2 - 5))) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ((long)(global_jiffies[get_stack_id()] + (slot << (10 + 2 - 5))) - (long)(twdr->twcal_timer.expires) < 0)))

    mod_timer(&twdr->twcal_timer,
       global_jiffies[get_stack_id()] + (slot << (10 + 2 - 5)));
   slot = (twdr->twcal_hand + slot) & ((1 << 5) - 1);
  }
...
void inet_twdr_twcal_tick(unsigned long data)
{
 ...
 for (n = 0; n < (1 << 5); n++) {
  if ((({ unsigned long __dummy; typeof(now) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ({ unsigned long __dummy; typeof(j) __dummy2; (void)(&__dummy == &__dummy2); 1; }) && ((long)(now) - (long)(j) >= 0))) {
   struct hlist_node *node, *safe;
   struct inet_timewait_sock *tw;

   for (node = (&twdr->twcal_row[slot])->first; node && ({ safe = node->next; 1; }) && ({ tw = ({ const typeof( ((typeof(*tw) *)0)->tw_death_node ) *__mptr = (node); (typeof(*tw) *)( (char *)__mptr - __builtin_offsetof(typeof(*tw),tw_death_node) );}); 1;}); node = safe) {
...
out:
 if ((twdr->tw_count -= killed) == 0)
  del_timer(&twdr->tw_timer);
 (({ (void)(0); ((*_GLOBAL_net_statistics_59_A[get_stack_id()])[0]); })->mibs[LINUX_MIB_TIMEWAITKILLED] += killed);
 do { do { } while (0); (void)0; (void)(&twdr->death_lock); } while (0);
}
