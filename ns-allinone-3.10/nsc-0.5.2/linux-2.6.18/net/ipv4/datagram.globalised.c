# 1 "linux-2.6.18/net/ipv4/datagram.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.18/net/ipv4/datagram.c"
# 14 "linux-2.6.18/net/ipv4/datagram.c"
...
# 15 "linux-2.6.18/net/ipv4/datagram.c" 2
...
# 16 "linux-2.6.18/net/ipv4/datagram.c" 2
...
# 17 "linux-2.6.18/net/ipv4/datagram.c" 2
...
# 18 "linux-2.6.18/net/ipv4/datagram.c" 2
# 1 "linux-2.6.18/include/net/ip.h" 1
# 29 "linux-2.6.18/include/net/ip.h"
...
# 19 "linux-2.6.18/net/ipv4/datagram.c" 2
...
# 21 "linux-2.6.18/net/ipv4/datagram.c" 2
...
int ip4_datagram_connect(struct sock *sk, struct sockaddr *uaddr, int addr_len)
{
 ...
 if (usin->sin_family != 2)
    return -97;

 sk_dst_reset(sk);

 oif = sk->__sk_common.skc_bound_dev_if;
 saddr = inet->saddr;
 if ((((usin->sin_addr.s_addr) & htonl(0xf0000000)) == htonl(0xe0000000))) {
  if (!oif)
   oif = inet->mc_index;
  if (!saddr)
   saddr = inet->mc_addr;
 ...
 err = ip_route_connect(&rt, usin->sin_addr.s_addr, saddr,
          (((inet_sk(sk)->tos)&0x1E) | sock_flag(sk, SOCK_LOCALROUTE)), oif,
          sk->sk_protocol,
          inet->sport, usin->sin_port, sk);
 if (err)
  return err;
 if ((rt->rt_flags & 0x10000000) && !sock_flag(sk, SOCK_BROADCAST)) {
  ip_rt_put(rt);
  return -13;
 }
   if (!inet->saddr)
 ...
 inet->id = global_jiffies[get_stack_id()];
