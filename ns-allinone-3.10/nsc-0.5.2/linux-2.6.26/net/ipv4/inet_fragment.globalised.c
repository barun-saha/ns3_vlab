# 1 "linux-2.6.26/net/ipv4/inet_fragment.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.26/net/ipv4/inet_fragment.c"
# 14 "linux-2.6.26/net/ipv4/inet_fragment.c"
...
# 15 "linux-2.6.26/net/ipv4/inet_fragment.c" 2
...
# 16 "linux-2.6.26/net/ipv4/inet_fragment.c" 2
...
{ }
# 17 "linux-2.6.26/net/ipv4/inet_fragment.c" 2
...
# 19 "linux-2.6.26/net/ipv4/inet_fragment.c" 2
...
# 20 "linux-2.6.26/net/ipv4/inet_fragment.c" 2
...
# 21 "linux-2.6.26/net/ipv4/inet_fragment.c" 2
...
typedef  const kernel_cap_t   _GLOBAL_117_T; extern  _GLOBAL_117_T  global___cap_empty_set[NUM_STACKS];    
typedef  const kernel_cap_t   _GLOBAL_118_T; extern  _GLOBAL_118_T  global___cap_full_set[NUM_STACKS];    
typedef  const kernel_cap_t   _GLOBAL_119_T; extern  _GLOBAL_119_T  global___cap_init_eff_set[NUM_STACKS];    
...
# 22 "linux-2.6.26/net/ipv4/inet_fragment.c" 2
...
# 24 "linux-2.6.26/net/ipv4/inet_fragment.c" 2
...
static void inet_frag_secret_rebuild(unsigned long dummy)
{
 ...
 do { do { } while (0); (void)0; (void)(&f->lock); } while (0);
 get_random_bytes(&f->rnd, sizeof(u32));
 for (i = 0; i < 64; i++) {
  struct inet_frag_queue *q;
  struct hlist_node *p, *n;

  for (p = (&f->hash[i])->first; p && ({ n = p->next; 1; }) && ({ q = ({ const typeof( ((typeof(*q) *)0)->list ) *__mptr = (p); (typeof(*q) *)( (char *)__mptr - __builtin_offsetof(typeof(*q),list) );}); 1;}); p = n) {
...
void inet_frags_init(struct inet_frags *f)
{
 ...
 for (i = 0; i < 64; i++)
  ((&f->hash[i])->first = ((void *)0));

 do { *(&f->lock) = (rwlock_t) { .raw_lock = { }, }; } while (0);

 f->rnd = (u32) ((global_num_physpages[get_stack_id()] ^ (global_num_physpages[get_stack_id()]>>7)) ^
       (global_jiffies[get_stack_id()] ^ (global_jiffies[get_stack_id()] >> 6)));

 setup_timer(&f->secret_timer, inet_frag_secret_rebuild,
   (unsigned long)f);
 f->secret_timer.expires = global_jiffies[get_stack_id()] + f->secret_interval;
...
static inline  __attribute__((always_inline)) void fq_unlink(struct inet_frag_queue *fq, struct inet_frags *f)
{
 do { do { } while (0); (void)0; (void)(&f->lock); } while (0);
 hlist_del(&fq->list);
...
static inline  __attribute__((always_inline)) void frag_kfree_skb(struct netns_frags *nf, struct inet_frags *f,
     ...
     int *work)
{
 ...
 do { if (__builtin_expect(!!(!(q->last_in & 4)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "q->last_in & INET_FRAG_COMPLETE", "linux-2.6.26/net/ipv4/inet_fragment.c" , 137); } } while(0);
 do { if (__builtin_expect(!!(!(del_timer(&q->timer) == 0)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "del_timer(&q->timer) == 0", "linux-2.6.26/net/ipv4/inet_fragment.c" , 138); } } while(0);
...
int inet_frag_evictor(struct netns_frags *nf, struct inet_frags *f)
{
 ...
 work = ((&nf->mem)->counter) - nf->low_thresh;
 while (work > 0) {
  do { do { } while (0); (void)0; (void)(&f->lock); } while (0);
  ...
  q = ({ const typeof( ((struct inet_frag_queue *)0)->lru_list ) *__mptr = ((&nf->lru_list)->next); (struct inet_frag_queue *)( (char *)__mptr - __builtin_offsetof(struct inet_frag_queue,lru_list) );});

  atomic_inc(&q->refcnt);
  do { do { } while (0); (void)0; (void)(&f->lock); } while (0);

  do { do { } while (0); (void)0; (void)(&q->lock); } while (0);
  if (!(q->last_in & 4))
   inet_frag_kill(q, f);
  do { do { } while (0); (void)0; (void)(&q->lock); } while (0);
  ...
  void *arg)
{
 ...
 hash = f->hashfn(qp_in);
# 225 "linux-2.6.26/net/ipv4/inet_fragment.c"
 qp = qp_in;
 if (!mod_timer(&qp->timer, global_jiffies[get_stack_id()] + nf->timeout))
  ...
  struct inet_frags *f, void *key, unsigned int hash)
{
 ...
 for (n = (&f->hash[hash])->first; n && ({ prefetch(n->next); 1;}) && ({ q = ({ const typeof( ((typeof(*q) *)0)->list ) *__mptr = (n); (typeof(*q) *)( (char *)__mptr - __builtin_offsetof(typeof(*q),list) );}); 1;}); n = n->next) {
