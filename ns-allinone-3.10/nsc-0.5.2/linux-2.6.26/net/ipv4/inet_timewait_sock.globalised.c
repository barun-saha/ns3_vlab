# 1 "linux-2.6.26/net/ipv4/inet_timewait_sock.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.26/net/ipv4/inet_timewait_sock.c"
# 11 "linux-2.6.26/net/ipv4/inet_timewait_sock.c"
...
# 12 "linux-2.6.26/net/ipv4/inet_timewait_sock.c" 2
# 1 "linux-2.6.26/include/net/inet_hashtables.h" 1
# 18 "linux-2.6.26/include/net/inet_hashtables.h"
...
# 19 "linux-2.6.26/include/net/inet_hashtables.h" 2
...
extern int inet_hash_connect(struct inet_timewait_death_row *death_row,
        struct sock *sk);
# 13 "linux-2.6.26/net/ipv4/inet_timewait_sock.c" 2
...
int ipv4_doint_and_flush_strategy(ctl_table *table, int *name, int nlen,
      void *oldval, size_t *oldlenp,
      void *newval, size_t newlen);
# 15 "linux-2.6.26/net/ipv4/inet_timewait_sock.c" 2
        ...
        struct inet_hashinfo *hashinfo)
{
 ...
 rwlock_t *lock = inet_ehash_lockp(hashinfo, tw->__tw_common.skc_hash);
      ...
      struct inet_hashinfo *hashinfo)
{
 ...
 rwlock_t *lock = inet_ehash_lockp(hashinfo, sk->__sk_common.skc_hash);
 struct inet_bind_hashbucket *bhead;
...
rescan:
 ...
 (({ (void)((0)); (((*_GLOBAL_net_statistics_12_A[get_stack_id()])[0])); })->mibs[LINUX_MIB_TIMEWAITED] += killed);
...
{
 struct inet_timewait_death_row *twdr =
  ({ const typeof( ((struct inet_timewait_death_row *)0)->twkill_work ) *__mptr = (work); (struct inet_timewait_death_row *)( (char *)__mptr - __builtin_offsetof(struct inet_timewait_death_row,twkill_work) );});
 int i;

 ((void)sizeof(char[1 - 2*!!((8 - 1) > (sizeof(twdr->thread_slots) * 8))]));
         ...
         const int timeo, const int timewait_len)
{
 struct hlist_head *list;
 int slot;
# 296 "linux-2.6.26/net/ipv4/inet_timewait_sock.c"
...
out:
 if ((twdr->tw_count -= killed) == 0)
  del_timer(&twdr->tw_timer);
 (({ (void)((0)); (((*_GLOBAL_net_statistics_12_A[get_stack_id()])[0])); })->mibs[LINUX_MIB_TIMEWAITKILLED] += killed);
 do { do { } while (0); (void)0; (void)(&twdr->death_lock); } while (0);
}
