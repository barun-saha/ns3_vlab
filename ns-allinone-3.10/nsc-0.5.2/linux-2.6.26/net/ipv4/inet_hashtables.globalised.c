# 1 "linux-2.6.26/net/ipv4/inet_hashtables.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.26/net/ipv4/inet_hashtables.c"
# 16 "linux-2.6.26/net/ipv4/inet_hashtables.c"
...
{ }
# 17 "linux-2.6.26/net/ipv4/inet_hashtables.c" 2
...
# 18 "linux-2.6.26/net/ipv4/inet_hashtables.c" 2
...
typedef  const kernel_cap_t   _GLOBAL_78_T; extern  _GLOBAL_78_T  global___cap_empty_set[NUM_STACKS];    
typedef  const kernel_cap_t   _GLOBAL_79_T; extern  _GLOBAL_79_T  global___cap_full_set[NUM_STACKS];    
typedef  const kernel_cap_t   _GLOBAL_80_T; extern  _GLOBAL_80_T  global___cap_init_eff_set[NUM_STACKS];    
...
typedef  int  _GLOBAL_81_T; extern  _GLOBAL_81_T  global_show_unhandled_signals[NUM_STACKS];   
...
# 19 "linux-2.6.26/net/ipv4/inet_hashtables.c" 2
...
typedef  int  _GLOBAL_107_T; extern  _GLOBAL_107_T  global_sysctl_nr_open[NUM_STACKS];   
...
typedef  struct files_stat_struct   _GLOBAL_108_T; extern  _GLOBAL_108_T  global_files_stat[NUM_STACKS];    
...
typedef  struct inodes_stat_t   _GLOBAL_109_T; extern  _GLOBAL_109_T  global_inodes_stat[NUM_STACKS];    

typedef  int  _GLOBAL_110_T; extern  _GLOBAL_110_T  global_leases_enable[NUM_STACKS]; extern  _GLOBAL_110_T  global_lease_break_time[NUM_STACKS];    
...
typedef  struct dentry_stat_t   _GLOBAL_111_T; extern  _GLOBAL_111_T  global_dentry_stat[NUM_STACKS];    
...
typedef  spinlock_t  _GLOBAL_112_T; extern  _GLOBAL_112_T  global_dcache_lock[NUM_STACKS];   
typedef  seqlock_t  _GLOBAL_113_T; extern  _GLOBAL_113_T  global_rename_lock[NUM_STACKS];   
...
typedef  int  _GLOBAL_114_T; extern  _GLOBAL_114_T  global_sysctl_vfs_cache_pressure[NUM_STACKS];   
...
typedef  struct kobject   _GLOBAL_121_T; extern  _GLOBAL_121_T  * global_fs_kobj[NUM_STACKS];    
...
typedef  struct kmem_cache   _GLOBAL_122_T; extern  _GLOBAL_122_T  * global_names_cachep[NUM_STACKS];    
...
typedef  const struct inode_operations    _GLOBAL_131_T; extern  _GLOBAL_131_T  global_page_symlink_inode_operations[NUM_STACKS];     
...
typedef  const struct inode_operations    _GLOBAL_133_T; extern  _GLOBAL_133_T  global_simple_dir_inode_operations[NUM_STACKS];     
...
extern int inet_csk_compat_setsockopt(struct sock *sk, int level, int optname,
          char *optval, int optlen);
# 23 "linux-2.6.26/net/ipv4/inet_hashtables.c" 2
...
extern int inet_hash_connect(struct inet_timewait_death_row *death_row,
        struct sock *sk);
# 24 "linux-2.6.26/net/ipv4/inet_hashtables.c" 2
...
int ipv4_doint_and_flush_strategy(ctl_table *table, int *name, int nlen,
      void *oldval, size_t *oldlenp,
      void *newval, size_t newlen);
# 25 "linux-2.6.26/net/ipv4/inet_hashtables.c" 2
...
{
 struct inet_hashinfo *table = sk->__sk_common.skc_prot->h.hashinfo;
...
;
# 118 "linux-2.6.26/net/ipv4/inet_hashtables.c"
           ...
           const int dif)
{
 ...
 for (node = (head)->first; node && ({ prefetch(node->next); 1;}) && ({ sk = ({ const typeof( ((typeof(*sk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - __builtin_offsetof(typeof(*sk),__sk_common.skc_node) );}); 1;}); node = node->next) {
  ...
  if (net_eq(sock_net(sk), net) && inet->num == hnum &&
    !((sk)->__sk_common.skc_family == 10 && (inet6_sk(sk)->ipv6only))) {
        ...
        const int dif)
{
 ...
 if (!hlist_empty(head)) {
  ...
  if (inet->num == hnum && !sk->__sk_common.skc_node.next &&
      ...
      !sk->__sk_common.skc_bound_dev_if && net_eq(sock_net(sk), net))
   goto sherry_cache;
      ...
      const int dif)
{
 ...
 for (node = (&head->chain)->first; node && ({ prefetch(node->next); 1;}) && ({ sk = ({ const typeof( ((typeof(*sk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - __builtin_offsetof(typeof(*sk),__sk_common.skc_node) );}); 1;}); node = node->next) {
  if ((((sk)->__sk_common.skc_hash == (hash)) && sock_net((sk)) == (net) && (inet_sk(sk)->daddr == (saddr)) && (inet_sk(sk)->rcv_saddr == (daddr)) && ((*((__portpair *)&(inet_sk(sk)->dport))) == (ports)) && (!((sk)->__sk_common.skc_bound_dev_if) || ((sk)->__sk_common.skc_bound_dev_if == (dif)))))
 ...
 for (node = (&head->twchain)->first; node && ({ prefetch(node->next); 1;}) && ({ sk = ({ const typeof( ((typeof(*sk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - __builtin_offsetof(typeof(*sk),__sk_common.skc_node) );}); 1;}); node = node->next) {
  if ((((sk)->__sk_common.skc_hash == (hash)) && sock_net((sk)) == (net) && (inet_twsk(sk)->tw_daddr == (saddr)) && (inet_twsk(sk)->tw_rcv_saddr == (daddr)) && ((*((__portpair *)&(inet_twsk(sk)->tw_dport))) == (ports)) && (!((sk)->__sk_common.skc_bound_dev_if) || ((sk)->__sk_common.skc_bound_dev_if == (dif)))))
        ...
        struct inet_timewait_sock **twp)
{
 ...
 for (node = (&head->twchain)->first; node && ({ prefetch(node->next); 1;}) && ({ sk2 = ({ const typeof( ((typeof(*sk2) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk2) *)( (char *)__mptr - __builtin_offsetof(typeof(*sk2),__sk_common.skc_node) );}); 1;}); node = node->next) {
  ...
  if ((((sk2)->__sk_common.skc_hash == (hash)) && sock_net((sk2)) == (net) && (inet_twsk(sk2)->tw_daddr == (saddr)) && (inet_twsk(sk2)->tw_rcv_saddr == (daddr)) && ((*((__portpair *)&(inet_twsk(sk2)->tw_dport))) == (ports)) && (!((sk2)->__sk_common.skc_bound_dev_if) || ((sk2)->__sk_common.skc_bound_dev_if == (dif))))) {
 ...
 for (node = (&head->chain)->first; node && ({ prefetch(node->next); 1;}) && ({ sk2 = ({ const typeof( ((typeof(*sk2) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk2) *)( (char *)__mptr - __builtin_offsetof(typeof(*sk2),__sk_common.skc_node) );}); 1;}); node = node->next) {
  if ((((sk2)->__sk_common.skc_hash == (hash)) && sock_net((sk2)) == (net) && (inet_sk(sk2)->daddr == (saddr)) && (inet_sk(sk2)->rcv_saddr == (daddr)) && ((*((__portpair *)&(inet_sk(sk2)->dport))) == (ports)) && (!((sk2)->__sk_common.skc_bound_dev_if) || ((sk2)->__sk_common.skc_bound_dev_if == (dif)))))
...
unique:
 ...
 if (twp) {
  *twp = tw;
  (({ (void)((0)); (((*_GLOBAL_net_statistics_12_A[get_stack_id()])[0])); })->mibs[LINUX_MIB_TIMEWAITRECYCLED]++);
 } else if (tw) {
...
void __inet_hash_nolisten(struct sock *sk)
{
 ...
 do { if (__builtin_expect(!!(!(sk_unhashed(sk))), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "sk_unhashed(sk)", "linux-2.6.26/net/ipv4/inet_hashtables.c" , 343); } } while(0);

 sk->__sk_common.skc_hash = inet_sk_ehashfn(sk);
 head = inet_ehash_bucket(hashinfo, sk->__sk_common.skc_hash);
 list = &head->chain;
 lock = inet_ehash_lockp(hashinfo, sk->__sk_common.skc_hash);
...
static void __inet_hash(struct sock *sk)
{
 ...
 do { if (__builtin_expect(!!(!(sk_unhashed(sk))), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "sk_unhashed(sk)", "linux-2.6.26/net/ipv4/inet_hashtables.c" , 368); } } while(0);
  ...
  void (*hash)(struct sock *sk))
{
 ...
 if (!snum) {
  ...
  typedef  u32  _GLOBAL_220_T; static  _GLOBAL_220_T  global_hint[NUM_STACKS];   
  u32 offset = global_hint[get_stack_id()] + port_offset;
  struct hlist_node *node;
...
ok:
  ...
  if (sk_unhashed(sk)) {
   inet_sk(sk)->sport = (( __be16)__fswab16((port)));
   hash(sk);
  }
  do { do { } while (0); (void)0; (void)(&head->lock); } while (0);
 ...
 if (sk_head(&tb->owners) == sk && !sk->__sk_common.skc_bind_node.next) {
  ...
  ret = check_established(death_row, sk, snum, ((void *)0));
out:
  local_bh_enable();
  return ret;
 }
