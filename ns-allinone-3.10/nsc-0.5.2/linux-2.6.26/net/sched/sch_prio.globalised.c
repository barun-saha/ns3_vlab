# 1 "linux-2.6.26/net/sched/sch_prio.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.26/net/sched/sch_prio.c"
# 14 "linux-2.6.26/net/sched/sch_prio.c"
...
{ }
# 15 "linux-2.6.26/net/sched/sch_prio.c" 2
...
# 20 "linux-2.6.26/net/sched/sch_prio.c" 2
...
# 21 "linux-2.6.26/net/sched/sch_prio.c" 2
...
# 22 "linux-2.6.26/net/sched/sch_prio.c" 2
...
# 55 "linux-2.6.26/net/sched/sch_prio.c"
  if (!q->filter_list || err < 0) {
   if (((band)&(0xFFFF0000U)))
    band = 0;
   band = q->prio2band[band&15];
   goto out;
  }
  band = res.classid;
 }
...
# 89 "linux-2.6.26/net/sched/sch_prio.c"
...
prio_requeue(struct sk_buff *skb, struct Qdisc* sch)
{
 ...
 qdisc = prio_classify(skb, sch, &ret);
# 116 "linux-2.6.26/net/sched/sch_prio.c"
...
static int prio_tune(struct Qdisc *sch, struct nlattr *opt)
{
 ...
 struct nlattr *tb[(__TCA_PRIO_MAX - 1) + 1];
 int err;
 int i;

 err = ({ qopt = nla_len(opt) >= sizeof(*qopt) ? nla_data(opt) : ((void *)0); __nla_parse_nested_compat(tb, (__TCA_PRIO_MAX - 1), opt, ((void *)0), sizeof(*qopt)); });
 ...
 for (i=0; i<q->bands; i++) {
  if (q->queues[i] == &global_noop_qdisc[get_stack_id()]) {
   ...
   child = qdisc_create_dflt(sch->dev, &global_pfifo_qdisc_ops[get_stack_id()],
        (((sch->handle)&(0xFFFF0000U))|((i + 1)&(0x0000FFFFU))));
   if (child) {
    qdisc_lock_tree((sch)->dev);
...
typedef  struct Qdisc_ops   _GLOBAL_151_T; static  _GLOBAL_151_T  global_rr_qdisc_ops[NUM_STACKS] __attribute__((__section__(".data.read_mostly")))   = {  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[0]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[1]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[2]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[3]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[4]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[5]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[6]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[7]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[8]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[9]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[10]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[11]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[12]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[13]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[14]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[15]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[16]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[17]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[18]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[19]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[20]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[21]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[22]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[23]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[24]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[25]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[26]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[27]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[28]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[29]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[30]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[31]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[32]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[33]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[34]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[35]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[36]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[37]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[38]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[39]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[40]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[41]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[42]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[43]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[44]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[45]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[46]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[47]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[48]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . next  = ( ( void *  ) 0  )   , . cl_ops  = & global_prio_class_ops[49]    , . id  = "rr"   , . priv_size  = sizeof ( struct prio_sched_data  )    , . enqueue  = prio_enqueue   , . dequeue  = rr_dequeue   , . requeue  = prio_requeue   , . drop  = prio_drop   , . init  = prio_init   , . reset  = prio_reset   , . destroy  = prio_destroy   , . change  = prio_tune   , . dump  = prio_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  };       
...
static int   __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) prio_module_init(void)
{
 int err;

 err = register_qdisc(&global_prio_qdisc_ops[get_stack_id()]);
 if (err < 0)
  return err;
 err = register_qdisc(&global_rr_qdisc_ops[get_stack_id()]);
 if (err < 0)
  unregister_qdisc(&global_prio_qdisc_ops[get_stack_id()]);
 return err;
}

static void    __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) prio_module_exit(void)
{
 unregister_qdisc(&global_prio_qdisc_ops[get_stack_id()]);
 unregister_qdisc(&global_rr_qdisc_ops[get_stack_id()]);
}

typedef  initcall_t  _GLOBAL_152_T; static  _GLOBAL_152_T  __initcall_prio_module_init6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init")))  = prio_module_init ;       
typedef  exitcall_t  _GLOBAL_153_T; static  _GLOBAL_153_T  global___exitcall_prio_module_exit[NUM_STACKS] __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit")))   = {  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  prio_module_exit,  };       
