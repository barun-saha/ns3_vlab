# 1 "linux-2.6.26/net/sched/sch_cbq.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.26/net/sched/sch_cbq.c"
# 13 "linux-2.6.26/net/sched/sch_cbq.c"
...
{ }
# 14 "linux-2.6.26/net/sched/sch_cbq.c" 2
...
# 19 "linux-2.6.26/net/sched/sch_cbq.c" 2
...
# 20 "linux-2.6.26/net/sched/sch_cbq.c" 2
...
# 21 "linux-2.6.26/net/sched/sch_cbq.c" 2
# 71 "linux-2.6.26/net/sched/sch_cbq.c"
...
# 225 "linux-2.6.26/net/sched/sch_cbq.c"
...
# 275 "linux-2.6.26/net/sched/sch_cbq.c"
  if (cl->level == 0)
   return cl;
...
static void cbq_ovl_classic(struct cbq_class *cl)
{
 ...
 if (!cl->delayed) {
  delay += cl->offtime;
# 466 "linux-2.6.26/net/sched/sch_cbq.c"
  if (cl->avgidle < 0)
...
static void cbq_ovl_delay(struct cbq_class *cl)
{
 ...
 if (!cl->delayed) {
  ...
  if (delay > 0) {
   ...
   expires = ktime_add_ns(expires, ((s64)(sched) << 10));
...
{
 struct cbq_sched_data *q = ({ const typeof( ((struct cbq_sched_data *)0)->delay_timer ) *__mptr = (timer); (struct cbq_sched_data *)( (char *)__mptr - __builtin_offsetof(struct cbq_sched_data,delay_timer) );});
 ...
 if (delay) {
  ...
  time = ktime_set(0, 0);
  time = ktime_add_ns(time, ((s64)(now + delay) << 10));
...
# 709 "linux-2.6.26/net/sched/sch_cbq.c"
...
cbq_update(struct cbq_sched_data *q)
{
 ...
 for ( ; cl; cl = cl->share) {
  ...
  cl->bstats.packets++;
  cl->bstats.bytes += len;
# 754 "linux-2.6.26/net/sched/sch_cbq.c"
  idle = q->now - cl->last;
  if ((unsigned long)idle > 128*1024*1024) {
   avgidle = cl->maxidle;
  } else {
   idle -= qdisc_l2t((cl)->R_tab,len);
  ...
  if (avgidle <= 0) {
   ...
   cl->avgidle = avgidle;
# 784 "linux-2.6.26/net/sched/sch_cbq.c"
   idle = (-avgidle) - ((-avgidle) >> cl->ewma_log);
# 795 "linux-2.6.26/net/sched/sch_cbq.c"
   idle -= qdisc_l2t((&q->link)->R_tab,len);
   idle += qdisc_l2t((cl)->R_tab,len);
...
cbq_under_limit(struct cbq_class *cl)
{
 ...
 if (cl->undertime == 0 || q->now >= cl->undertime) {
  cl->delayed = 0;
  return cl;
 }

 do {
# 839 "linux-2.6.26/net/sched/sch_cbq.c"
  ...
  if (cl->level > q->toplevel)
   return ((void *)0);
 } while (cl->undertime != 0 && q->now < cl->undertime);
...
cbq_dequeue(struct Qdisc *sch)
{
 ...
 if (q->tx_class) {
  ...
  incr2 = qdisc_l2t((&q->link)->R_tab,q->tx_len);
  q->now += incr2;
  cbq_update(q);
  if ((incr -= incr2) < 0)
   incr = 0;
...
# 1030 "linux-2.6.26/net/sched/sch_cbq.c"
  if (q->toplevel == 8 &&
      q->link.undertime == 0)
   break;
...
# 1374 "linux-2.6.26/net/sched/sch_cbq.c"
...
typedef  const struct nla_policy    _GLOBAL_149_T; static  _GLOBAL_149_T  _GLOBAL_0_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_1_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_2_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_3_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_4_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_5_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_6_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_7_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_8_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_9_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_10_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_11_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_12_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_13_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_14_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_15_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_16_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_17_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_18_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_19_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_20_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_21_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_22_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_23_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_24_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_25_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_26_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_27_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_28_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_29_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_30_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_31_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_32_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_33_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_34_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_35_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_36_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_37_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_38_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_39_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_40_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_41_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_42_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_43_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_44_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_45_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_46_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_47_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_48_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static  _GLOBAL_149_T  _GLOBAL_49_cbq_policy_I [ ( __TCA_CBQ_MAX - 1  )  + 1  ]  = { [ TCA_CBQ_LSSOPT ] = { . len  = sizeof ( struct tc_cbq_lssopt  )   }  , [ TCA_CBQ_WRROPT ] = { . len  = sizeof ( struct tc_cbq_wrropt  )   }   , [ TCA_CBQ_FOPT ] = { . len  = sizeof ( struct tc_cbq_fopt  )   }   , [ TCA_CBQ_OVL_STRATEGY ] = { . len  = sizeof ( struct tc_cbq_ovl  )   }   , [ TCA_CBQ_RATE ] = { . len  = sizeof ( struct tc_ratespec  )   }   , [ TCA_CBQ_RTAB ] = { . type  = NLA_BINARY  , . len  = 1024   }   , [ TCA_CBQ_POLICE ] = { . len  = sizeof ( struct tc_cbq_police  )   }   , }  ; static __attribute__ ((unused)) __typeof__(_GLOBAL_0_cbq_policy_I) *_GLOBAL_cbq_policy_8_A[NUM_STACKS] = { &_GLOBAL_0_cbq_policy_I, &_GLOBAL_1_cbq_policy_I, &_GLOBAL_2_cbq_policy_I, &_GLOBAL_3_cbq_policy_I, &_GLOBAL_4_cbq_policy_I, &_GLOBAL_5_cbq_policy_I, &_GLOBAL_6_cbq_policy_I, &_GLOBAL_7_cbq_policy_I, &_GLOBAL_8_cbq_policy_I, &_GLOBAL_9_cbq_policy_I, &_GLOBAL_10_cbq_policy_I, &_GLOBAL_11_cbq_policy_I, &_GLOBAL_12_cbq_policy_I, &_GLOBAL_13_cbq_policy_I, &_GLOBAL_14_cbq_policy_I, &_GLOBAL_15_cbq_policy_I, &_GLOBAL_16_cbq_policy_I, &_GLOBAL_17_cbq_policy_I, &_GLOBAL_18_cbq_policy_I, &_GLOBAL_19_cbq_policy_I, &_GLOBAL_20_cbq_policy_I, &_GLOBAL_21_cbq_policy_I, &_GLOBAL_22_cbq_policy_I, &_GLOBAL_23_cbq_policy_I, &_GLOBAL_24_cbq_policy_I, &_GLOBAL_25_cbq_policy_I, &_GLOBAL_26_cbq_policy_I, &_GLOBAL_27_cbq_policy_I, &_GLOBAL_28_cbq_policy_I, &_GLOBAL_29_cbq_policy_I, &_GLOBAL_30_cbq_policy_I, &_GLOBAL_31_cbq_policy_I, &_GLOBAL_32_cbq_policy_I, &_GLOBAL_33_cbq_policy_I, &_GLOBAL_34_cbq_policy_I, &_GLOBAL_35_cbq_policy_I, &_GLOBAL_36_cbq_policy_I, &_GLOBAL_37_cbq_policy_I, &_GLOBAL_38_cbq_policy_I, &_GLOBAL_39_cbq_policy_I, &_GLOBAL_40_cbq_policy_I, &_GLOBAL_41_cbq_policy_I, &_GLOBAL_42_cbq_policy_I, &_GLOBAL_43_cbq_policy_I, &_GLOBAL_44_cbq_policy_I, &_GLOBAL_45_cbq_policy_I, &_GLOBAL_46_cbq_policy_I, &_GLOBAL_47_cbq_policy_I, &_GLOBAL_48_cbq_policy_I, &_GLOBAL_49_cbq_policy_I, };          
...
{
 struct cbq_sched_data *q = qdisc_priv(sch);
 struct nlattr *tb[(__TCA_CBQ_MAX - 1) + 1];
 struct tc_ratespec *r;
 int err;

 err = nla_parse_nested(tb, (__TCA_CBQ_MAX - 1), opt, (*_GLOBAL_cbq_policy_8_A[get_stack_id()]));
 if (err < 0)
  return err;
 ...
 hrtimer_init(&q->delay_timer, 1, HRTIMER_MODE_ABS);
...
static __inline__  __attribute__((always_inline)) int cbq_dump_rate(struct sk_buff *skb, struct cbq_class *cl)
{
 ...
 do { if (__builtin_expect(!!(nla_put(skb, TCA_CBQ_RATE, sizeof(cl->R_tab->rate), &cl->R_tab->rate) < 0), 0)) goto nla_put_failure; } while(0);
 return skb->len;
...
static __inline__  __attribute__((always_inline)) int cbq_dump_lss(struct sk_buff *skb, struct cbq_class *cl)
{
 ...
 opt.change = ~0;
 do { if (__builtin_expect(!!(nla_put(skb, TCA_CBQ_LSSOPT, sizeof(opt), &opt) < 0), 0)) goto nla_put_failure; } while(0);
 return skb->len;
...
static __inline__  __attribute__((always_inline)) int cbq_dump_wrr(struct sk_buff *skb, struct cbq_class *cl)
{
 ...
 do { if (__builtin_expect(!!(nla_put(skb, TCA_CBQ_WRROPT, sizeof(opt), &opt) < 0), 0)) goto nla_put_failure; } while(0);
 return skb->len;
...
static __inline__  __attribute__((always_inline)) int cbq_dump_ovl(struct sk_buff *skb, struct cbq_class *cl)
{
 ...
 do { if (__builtin_expect(!!(nla_put(skb, TCA_CBQ_OVL_STRATEGY, sizeof(opt), &opt) < 0), 0)) goto nla_put_failure; } while(0);
 return skb->len;
...
static __inline__  __attribute__((always_inline)) int cbq_dump_fopt(struct sk_buff *skb, struct cbq_class *cl)
{
 ...
 if (cl->split || cl->defmap) {
  ...
  opt.defchange = ~0;
  do { if (__builtin_expect(!!(nla_put(skb, TCA_CBQ_FOPT, sizeof(opt), &opt) < 0), 0)) goto nla_put_failure; } while(0);
 }
 return skb->len;
...
# 1556 "linux-2.6.26/net/sched/sch_cbq.c"
       ...
       struct Qdisc **old)
{
 ...
 if (cl) {
  if (new == ((void *)0)) {
   if ((new = qdisc_create_dflt(sch->dev, &global_pfifo_qdisc_ops[get_stack_id()],
           cl->classid)) == ((void *)0))
    return -105;
  } else {
  ...
  *old = ((__typeof__(*(&cl->q)))__xchg((unsigned long)(new), (&cl->q), sizeof(*(&cl->q))));
...
cbq_destroy(struct Qdisc* sch)
{
 ...
 struct cbq_class *cl;
 unsigned h;
# 1730 "linux-2.6.26/net/sched/sch_cbq.c"
 for (h = 0; h < 16; h++) {
  for (cl = q->classes[h]; cl; cl = cl->next)
   tcf_destroy_chain(&cl->filter_list);
 }
...
static void cbq_put(struct Qdisc *sch, unsigned long arg)
{
 ...
 if (--cl->refcnt == 0) {
# 1758 "linux-2.6.26/net/sched/sch_cbq.c"
  cbq_destroy_class(sch, cl);
 }
}
   ...
   unsigned long *arg)
{
 ...
 rtab = ((void *)0);
 cl->refcnt = 1;
 if (!(cl->q = qdisc_create_dflt(sch->dev, &global_pfifo_qdisc_ops[get_stack_id()], classid)))
