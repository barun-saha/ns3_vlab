# 1 "linux-2.6.26/net/sched/sch_teql.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.26/net/sched/sch_teql.c"
# 11 "linux-2.6.26/net/sched/sch_teql.c"
...
{ }
# 12 "linux-2.6.26/net/sched/sch_teql.c" 2




# 1 "linux-2.6.26/include/linux/if_arp.h" 1
# 26 "linux-2.6.26/include/linux/if_arp.h"
...
# 27 "linux-2.6.26/include/linux/if_arp.h" 2
...
# 17 "linux-2.6.26/net/sched/sch_teql.c" 2
...
# 22 "linux-2.6.26/net/sched/sch_teql.c" 2
...
# 24 "linux-2.6.26/net/sched/sch_teql.c" 2
# 55 "linux-2.6.26/net/sched/sch_teql.c"
struct teql_master
{
...
static int teql_qdisc_init(struct Qdisc *sch, struct nlattr *opt)
{
 ...
 if (m->slaves) {
  ...
  m->slaves = sch;
  m->dev->mtu = dev->mtu;
  m->dev->flags = (m->dev->flags&~(0x2|0x10))|(dev->flags&(0x2|0x10));
 }
 return 0;
}
...
__teql_resolve(struct sk_buff *skb, struct sk_buff *skb_res, struct net_device *dev)
{
 ...
 if (neigh_event_send(n, skb_res) == 0) {
  ...
  err = dev_hard_header(skb, dev, __fswab16(( __u16)(__be16)(skb->protocol)),
          n->ha, ((void *)0), skb->len);
  do { do { } while (0); (void)0; (void)(&n->lock); } while (0);
...
static inline  __attribute__((always_inline)) int teql_resolve(struct sk_buff *skb,
...
static int teql_master_open(struct net_device *dev)
{
 ...
 m->dev->mtu = mtu;
 m->dev->flags = (m->dev->flags&~(0x2|0x10)) | flags;
...
static   __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) void teql_master_setup(struct net_device *dev)
{
 struct teql_master *master = netdev_priv(dev);
...
static int   __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) teql_init(void)
{
 ...
 for (i = 0; i < global_max_equalizers[get_stack_id()]; i++) {
  ...
  dev = alloc_netdev_mq(sizeof(struct teql_master), "teql%d", teql_master_setup, 1);
...
static void    __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) teql_exit(void)
{
