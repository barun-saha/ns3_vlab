# 1 "linux-2.6.26/net/sched/sch_gred.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.26/net/sched/sch_gred.c"
# 21 "linux-2.6.26/net/sched/sch_gred.c"
...
{ }
# 22 "linux-2.6.26/net/sched/sch_gred.c" 2
...
# 25 "linux-2.6.26/net/sched/sch_gred.c" 2
...
# 26 "linux-2.6.26/net/sched/sch_gred.c" 2
...
# 7 "linux-2.6.26/include/net/red.h" 2
...
# 27 "linux-2.6.26/net/sched/sch_gred.c" 2
...
static inline  __attribute__((always_inline)) int gred_wred_mode(struct gred_sched *table)
{
 return (__builtin_constant_p((GRED_WRED_MODE)) ? constant_test_bit((GRED_WRED_MODE), (&table->flags)) : variable_test_bit((GRED_WRED_MODE), (&table->flags)));
}

static inline  __attribute__((always_inline)) void gred_enable_wred_mode(struct gred_sched *table)
{
 __set_bit(GRED_WRED_MODE, &table->flags);
}

static inline  __attribute__((always_inline)) void gred_disable_wred_mode(struct gred_sched *table)
{
 __clear_bit(GRED_WRED_MODE, &table->flags);
}

static inline  __attribute__((always_inline)) int gred_rio_mode(struct gred_sched *table)
{
 return (__builtin_constant_p((GRED_RIO_MODE)) ? constant_test_bit((GRED_RIO_MODE), (&table->flags)) : variable_test_bit((GRED_RIO_MODE), (&table->flags)));
}

static inline  __attribute__((always_inline)) void gred_enable_rio_mode(struct gred_sched *table)
{
 __set_bit(GRED_RIO_MODE, &table->flags);
}

static inline  __attribute__((always_inline)) void gred_disable_rio_mode(struct gred_sched *table)
{
 __clear_bit(GRED_RIO_MODE, &table->flags);
}

static inline  __attribute__((always_inline)) int gred_wred_mode_check(struct Qdisc *sch)
{
...
static inline  __attribute__((always_inline)) unsigned int gred_backlog(struct gred_sched *table,
...
static inline  __attribute__((always_inline)) u16 tc_index_to_dp(struct sk_buff *skb)
{
 return skb->tc_index & (16 - 1);
}

static inline  __attribute__((always_inline)) void gred_load_wred_set(struct gred_sched *table,
          struct gred_sched_data *q)
{
...
static inline  __attribute__((always_inline)) void gred_store_wred_set(struct gred_sched *table,
           struct gred_sched_data *q)
{
 table->wred_set.qavg = q->parms.qavg;
}

static inline  __attribute__((always_inline)) int gred_use_ecn(struct gred_sched *t)
{
 return t->red_flags & 1;
}

static inline  __attribute__((always_inline)) int gred_use_harddrop(struct gred_sched *t)
{
 return t->red_flags & 2;
}
...
static int gred_enqueue(struct sk_buff *skb, struct Qdisc* sch)
{
 ...
 if (dp >= t->DPs || (q = t->tab[dp]) == ((void *)0)) {
  ...
  skb->tc_index = (skb->tc_index & ~(16 - 1)) | dp;
 }
...
static int gred_requeue(struct sk_buff *skb, struct Qdisc* sch)
{
 ...
 if (dp >= t->DPs || (q = t->tab[dp]) == ((void *)0)) {
  if (net_ratelimit())
   printk("<4>" "GRED: Unable to relocate VQ 0x%x "
...
static struct sk_buff *gred_dequeue(struct Qdisc* sch)
{
 ...
 if (skb) {
  ...
  if (dp >= t->DPs || (q = t->tab[dp]) == ((void *)0)) {
   if (net_ratelimit())
    printk("<4>" "GRED: Unable to relocate "
...
static inline  __attribute__((always_inline)) void gred_destroy_vq(struct gred_sched_data *q)
{
 kfree(q);
}

static inline  __attribute__((always_inline)) int gred_change_table_def(struct Qdisc *sch, struct nlattr *dps)
{
 struct gred_sched *table = qdisc_priv(sch);
 ...
 if (sopt->DPs > 16 || sopt->DPs == 0 || sopt->def_DP >= sopt->DPs)
  return -22;
 ...
 for (i = table->DPs; i < 16; i++) {
  if (table->tab[i]) {
   printk("<4>" "GRED: Warning: Destroying "
          "shadowed VQ 0x%x\n", i);
...
static inline  __attribute__((always_inline)) int gred_change_vq(struct Qdisc *sch, int dp,
     struct tc_gred_qopt *ctl, int prio, u8 *stab)
{
 ...
 if (table->tab[dp] == ((void *)0)) {
  table->tab[dp] = kzalloc(sizeof(*q), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
  if (table->tab[dp] == ((void *)0))
   return -12;
 }
...
typedef  const struct nla_policy    _GLOBAL_208_T; static  _GLOBAL_208_T  _GLOBAL_0_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_1_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_2_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_3_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_4_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_5_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_6_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_7_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_8_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_9_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_10_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_11_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_12_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_13_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_14_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_15_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_16_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_17_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_18_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_19_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_20_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_21_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_22_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_23_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_24_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_25_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_26_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_27_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_28_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_29_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_30_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_31_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_32_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_33_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_34_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_35_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_36_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_37_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_38_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_39_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_40_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_41_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_42_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_43_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_44_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_45_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_46_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_47_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_48_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static  _GLOBAL_208_T  _GLOBAL_49_gred_policy_I [ ( __TCA_GRED_MAX - 1  )  + 1  ]  = { [ TCA_GRED_PARMS ] = { . len  = sizeof ( struct tc_gred_qopt  )   }  , [ TCA_GRED_STAB ] = { . len  = 256  }   , [ TCA_GRED_DPS ] = { . len  = sizeof ( struct tc_gred_sopt  )   }   , }  ; static __attribute__ ((unused)) __typeof__(_GLOBAL_0_gred_policy_I) *_GLOBAL_gred_policy_11_A[NUM_STACKS] = { &_GLOBAL_0_gred_policy_I, &_GLOBAL_1_gred_policy_I, &_GLOBAL_2_gred_policy_I, &_GLOBAL_3_gred_policy_I, &_GLOBAL_4_gred_policy_I, &_GLOBAL_5_gred_policy_I, &_GLOBAL_6_gred_policy_I, &_GLOBAL_7_gred_policy_I, &_GLOBAL_8_gred_policy_I, &_GLOBAL_9_gred_policy_I, &_GLOBAL_10_gred_policy_I, &_GLOBAL_11_gred_policy_I, &_GLOBAL_12_gred_policy_I, &_GLOBAL_13_gred_policy_I, &_GLOBAL_14_gred_policy_I, &_GLOBAL_15_gred_policy_I, &_GLOBAL_16_gred_policy_I, &_GLOBAL_17_gred_policy_I, &_GLOBAL_18_gred_policy_I, &_GLOBAL_19_gred_policy_I, &_GLOBAL_20_gred_policy_I, &_GLOBAL_21_gred_policy_I, &_GLOBAL_22_gred_policy_I, &_GLOBAL_23_gred_policy_I, &_GLOBAL_24_gred_policy_I, &_GLOBAL_25_gred_policy_I, &_GLOBAL_26_gred_policy_I, &_GLOBAL_27_gred_policy_I, &_GLOBAL_28_gred_policy_I, &_GLOBAL_29_gred_policy_I, &_GLOBAL_30_gred_policy_I, &_GLOBAL_31_gred_policy_I, &_GLOBAL_32_gred_policy_I, &_GLOBAL_33_gred_policy_I, &_GLOBAL_34_gred_policy_I, &_GLOBAL_35_gred_policy_I, &_GLOBAL_36_gred_policy_I, &_GLOBAL_37_gred_policy_I, &_GLOBAL_38_gred_policy_I, &_GLOBAL_39_gred_policy_I, &_GLOBAL_40_gred_policy_I, &_GLOBAL_41_gred_policy_I, &_GLOBAL_42_gred_policy_I, &_GLOBAL_43_gred_policy_I, &_GLOBAL_44_gred_policy_I, &_GLOBAL_45_gred_policy_I, &_GLOBAL_46_gred_policy_I, &_GLOBAL_47_gred_policy_I, &_GLOBAL_48_gred_policy_I, &_GLOBAL_49_gred_policy_I, };          
...
static int gred_change(struct Qdisc *sch, struct nlattr *opt)
{
 ...
 struct nlattr *tb[(__TCA_GRED_MAX - 1) + 1];
 int err, prio = (16 / 2);
 u8 *stab;
 ...
 err = nla_parse_nested(tb, (__TCA_GRED_MAX - 1), opt, (*_GLOBAL_gred_policy_11_A[get_stack_id()]));
 if (err < 0)
  return err;

 if (tb[TCA_GRED_PARMS] == ((void *)0) && tb[TCA_GRED_STAB] == ((void *)0))
  return gred_change_table_def(sch, opt);

 if (tb[TCA_GRED_PARMS] == ((void *)0) ||
     tb[TCA_GRED_STAB] == ((void *)0))
  return -22;
 ...
 if (gred_rio_mode(table)) {
  if (ctl->prio == 0) {
   ...
   printk("<7>" "GRED: DP %u does not have a prio "
...
static int gred_dump(struct Qdisc *sch, struct sk_buff *skb)
{
 ...
 do { if (__builtin_expect(!!(nla_put(skb, TCA_GRED_DPS, sizeof(sopt), &sopt) < 0), 0)) goto nla_put_failure; } while(0);
...
typedef  struct Qdisc_ops   _GLOBAL_209_T; static  _GLOBAL_209_T  global_gred_qdisc_ops[NUM_STACKS] __attribute__((__section__(".data.read_mostly")))   = {  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  { . id  = "gred"  , . priv_size  = sizeof ( struct gred_sched  )    , . enqueue  = gred_enqueue   , . dequeue  = gred_dequeue   , . requeue  = gred_requeue   , . drop  = gred_drop   , . init  = gred_init   , . reset  = gred_reset   , . destroy  = gred_destroy   , . change  = gred_change   , . dump  = gred_dump   , . owner  = ( ( struct module  *  ) 0  )    , } ,  };       
...
static int   __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) gred_module_init(void)
{
 return register_qdisc(&global_gred_qdisc_ops[get_stack_id()]);
}

static void    __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) gred_module_exit(void)
{
 unregister_qdisc(&global_gred_qdisc_ops[get_stack_id()]);
}

typedef  initcall_t  _GLOBAL_210_T; static  _GLOBAL_210_T  __initcall_gred_module_init6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init")))  = gred_module_init ;       
typedef  exitcall_t  _GLOBAL_211_T; static  _GLOBAL_211_T  global___exitcall_gred_module_exit[NUM_STACKS] __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit")))   = {  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  gred_module_exit,  };       
