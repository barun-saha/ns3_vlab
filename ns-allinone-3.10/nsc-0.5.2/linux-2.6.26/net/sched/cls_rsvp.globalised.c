# 1 "linux-2.6.26/net/sched/cls_rsvp.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.26/net/sched/cls_rsvp.c"
# 12 "linux-2.6.26/net/sched/cls_rsvp.c"
...
{ }
# 13 "linux-2.6.26/net/sched/cls_rsvp.c" 2
...
# 18 "linux-2.6.26/net/sched/cls_rsvp.c" 2
...
typedef  atomic_t  _GLOBAL_100_T; extern  _GLOBAL_100_T  global_flow_cache_genid[NUM_STACKS];   
...
int ipv4_doint_and_flush_strategy(ctl_table *table, int *name, int nlen,
      void *oldval, size_t *oldlenp,
      void *newval, size_t newlen);
# 19 "linux-2.6.26/net/sched/cls_rsvp.c" 2

# 1 "linux-2.6.26/include/net/act_api.h" 1
...
# 9 "linux-2.6.26/include/net/act_api.h" 2
...
# 10 "linux-2.6.26/include/net/act_api.h" 2
...
# 36 "linux-2.6.26/include/net/act_api.h"
struct tcf_police {
...
static inline  __attribute__((always_inline)) unsigned int tcf_hash(u32 index, unsigned int hmask)
{
 return index & hmask;
}
# 21 "linux-2.6.26/net/sched/cls_rsvp.c" 2
# 1 "linux-2.6.26/include/net/pkt_cls.h" 1
# 10 "linux-2.6.26/include/net/pkt_cls.h"
struct tcf_walker
{
 int stop;
 int skip;
...
# 87 "linux-2.6.26/include/net/pkt_cls.h"
static inline  __attribute__((always_inline)) int
...
# 121 "linux-2.6.26/include/net/pkt_cls.h"
static inline  __attribute__((always_inline)) int
...
# 305 "linux-2.6.26/include/net/pkt_cls.h"
struct tcf_ematch_tree
{
};
# 317 "linux-2.6.26/include/net/pkt_cls.h"
...
static inline  __attribute__((always_inline)) int tcf_valid_offset(const struct sk_buff *skb,
       const unsigned char *ptr, const int len)
{
 return __builtin_expect(!!((ptr + len) < skb_tail_pointer(skb) && ptr > skb->head), 0);
}
# 22 "linux-2.6.26/net/sched/cls_rsvp.c" 2





# 1 "linux-2.6.26/net/sched/cls_rsvp.h" 1
# 69 "linux-2.6.26/net/sched/cls_rsvp.h"
struct rsvp_head
{
 u32 tmap[256/32];
 u32 hgenerator;
...
static __inline__  __attribute__((always_inline)) unsigned hash_dst(__be32 *dst, u8 protocol, u8 tunnelid)
{
 unsigned h = ( __u32)dst[1 -1];
 h ^= h>>16;
 h ^= h>>8;
 return (h ^ protocol ^ tunnelid) & 0xFF;
}

static __inline__  __attribute__((always_inline)) unsigned hash_src(__be32 *src)
{
 unsigned h = ( __u32)src[1 -1];
 h ^= h>>16;
 h ^= h>>8;
 h ^= h>>4;
 return h & 0xF;
...
typedef  struct tcf_ext_map   _GLOBAL_222_T; static  _GLOBAL_222_T  global_rsvp_ext_map[NUM_STACKS] = {  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  { . police  = TCA_RSVP_POLICE  , . action  = TCA_RSVP_ACT   } ,  };      
   
   

# 134 "linux-2.6.26/net/sched/cls_rsvp.h"
...
restart:
 ...
 if (nhptr->frag_off & (( __be16)__fswab16((0x2000|0x1FFF))))
  return -1;
...
typedef  const struct nla_policy    _GLOBAL_223_T; static  _GLOBAL_223_T  _GLOBAL_0_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_1_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_2_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_3_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_4_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_5_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_6_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_7_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_8_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_9_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_10_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_11_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_12_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_13_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_14_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_15_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_16_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_17_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_18_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_19_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_20_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_21_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_22_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_23_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_24_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_25_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_26_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_27_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_28_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_29_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_30_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_31_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_32_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_33_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_34_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_35_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_36_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_37_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_38_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_39_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_40_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_41_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_42_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_43_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_44_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_45_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_46_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_47_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_48_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static  _GLOBAL_223_T  _GLOBAL_49_rsvp_policy_I [ ( __TCA_RSVP_MAX - 1  )  + 1  ]  = { [ TCA_RSVP_CLASSID ] = { . type  = NLA_U32  }  , [ TCA_RSVP_DST ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_SRC ] = { . type  = NLA_BINARY  , . len  = 1 * sizeof ( u32 )     }   , [ TCA_RSVP_PINFO ] = { . len  = sizeof ( struct tc_rsvp_pinfo  )   }   , }  ; static __attribute__ ((unused)) __typeof__(_GLOBAL_0_rsvp_policy_I) *_GLOBAL_rsvp_policy_14_A[NUM_STACKS] = { &_GLOBAL_0_rsvp_policy_I, &_GLOBAL_1_rsvp_policy_I, &_GLOBAL_2_rsvp_policy_I, &_GLOBAL_3_rsvp_policy_I, &_GLOBAL_4_rsvp_policy_I, &_GLOBAL_5_rsvp_policy_I, &_GLOBAL_6_rsvp_policy_I, &_GLOBAL_7_rsvp_policy_I, &_GLOBAL_8_rsvp_policy_I, &_GLOBAL_9_rsvp_policy_I, &_GLOBAL_10_rsvp_policy_I, &_GLOBAL_11_rsvp_policy_I, &_GLOBAL_12_rsvp_policy_I, &_GLOBAL_13_rsvp_policy_I, &_GLOBAL_14_rsvp_policy_I, &_GLOBAL_15_rsvp_policy_I, &_GLOBAL_16_rsvp_policy_I, &_GLOBAL_17_rsvp_policy_I, &_GLOBAL_18_rsvp_policy_I, &_GLOBAL_19_rsvp_policy_I, &_GLOBAL_20_rsvp_policy_I, &_GLOBAL_21_rsvp_policy_I, &_GLOBAL_22_rsvp_policy_I, &_GLOBAL_23_rsvp_policy_I, &_GLOBAL_24_rsvp_policy_I, &_GLOBAL_25_rsvp_policy_I, &_GLOBAL_26_rsvp_policy_I, &_GLOBAL_27_rsvp_policy_I, &_GLOBAL_28_rsvp_policy_I, &_GLOBAL_29_rsvp_policy_I, &_GLOBAL_30_rsvp_policy_I, &_GLOBAL_31_rsvp_policy_I, &_GLOBAL_32_rsvp_policy_I, &_GLOBAL_33_rsvp_policy_I, &_GLOBAL_34_rsvp_policy_I, &_GLOBAL_35_rsvp_policy_I, &_GLOBAL_36_rsvp_policy_I, &_GLOBAL_37_rsvp_policy_I, &_GLOBAL_38_rsvp_policy_I, &_GLOBAL_39_rsvp_policy_I, &_GLOBAL_40_rsvp_policy_I, &_GLOBAL_41_rsvp_policy_I, &_GLOBAL_42_rsvp_policy_I, &_GLOBAL_43_rsvp_policy_I, &_GLOBAL_44_rsvp_policy_I, &_GLOBAL_45_rsvp_policy_I, &_GLOBAL_46_rsvp_policy_I, &_GLOBAL_47_rsvp_policy_I, &_GLOBAL_48_rsvp_policy_I, &_GLOBAL_49_rsvp_policy_I, };           
         ...
         unsigned long *arg)
{
 ...
 struct nlattr *tb[(__TCA_RSVP_MAX - 1 ) + 1];
 struct tcf_exts e;
 unsigned h1, h2;
 __be32 *dst;
 int err;
 ...
 err = nla_parse_nested(tb, (__TCA_RSVP_MAX - 1 ), opt, (*_GLOBAL_rsvp_policy_14_A[get_stack_id()]));
 if (err < 0)
  return err;
 ...
 if (tb[TCA_RSVP_SRC-1]) {
  (__builtin_constant_p(sizeof(f->src)) ? __constant_memcpy((f->src),(nla_data(tb[TCA_RSVP_SRC-1])),(sizeof(f->src))) : __memcpy((f->src),(nla_data(tb[TCA_RSVP_SRC-1])),(sizeof(f->src))));
  h2 = hash_src(f->src);
 }
       ...
       struct sk_buff *skb, struct tcmsg *t)
{
 ...
 do { if (__builtin_expect(!!(nla_put(skb, TCA_RSVP_DST, sizeof(s->dst), &s->dst) < 0), 0)) goto nla_put_failure; } while(0);
 pinfo.dpi = s->dpi;
 pinfo.spi = f->spi;
 ...
 pinfo.pad = 0;
 do { if (__builtin_expect(!!(nla_put(skb, TCA_RSVP_PINFO, sizeof(pinfo), &pinfo) < 0), 0)) goto nla_put_failure; } while(0);
 if (f->res.classid)
  do { u32 __tmp = f->res.classid; do { if (__builtin_expect(!!(nla_put(skb, TCA_RSVP_CLASSID, sizeof(u32), &__tmp) < 0), 0)) goto nla_put_failure; } while(0); } while(0);
 if (((f->handle>>8)&0xFF) != 16)
  do { if (__builtin_expect(!!(nla_put(skb, TCA_RSVP_SRC, sizeof(f->src), f->src) < 0), 0)) goto nla_put_failure; } while(0);
...
static int   __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) init_rsvp(void)
{
 return register_tcf_proto_ops(&global_cls_rsvp_ops[get_stack_id()]);
}

static void    __attribute__ ((__section__(".exit.text"))) __attribute__((__used__)) __attribute__((__cold__)) exit_rsvp(void)
{
 unregister_tcf_proto_ops(&global_cls_rsvp_ops[get_stack_id()]);
}

typedef  initcall_t  _GLOBAL_225_T; static  _GLOBAL_225_T  __initcall_init_rsvp6 __attribute__((__used__)) __attribute__((__section__(".initcall" "6" ".init")))  = init_rsvp ;       
typedef  exitcall_t  _GLOBAL_226_T; static  _GLOBAL_226_T  global___exitcall_exit_rsvp[NUM_STACKS] __attribute__((__used__)) __attribute__ ((__section__(".exitcall.exit")))   = {  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  exit_rsvp,  };       
# 28 "linux-2.6.26/net/sched/cls_rsvp.c" 2
;
