# 1 "linux-2.6.26/net/core/ethtool.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.26/net/core/ethtool.c"
# 14 "linux-2.6.26/net/core/ethtool.c"
...
{ }
# 15 "linux-2.6.26/net/core/ethtool.c" 2
...
# 17 "linux-2.6.26/net/core/ethtool.c" 2
...
# 19 "linux-2.6.26/net/core/ethtool.c" 2
...
# 20 "linux-2.6.26/net/core/ethtool.c" 2
# 1 "linux-2.6.26/include/asm/uaccess.h" 1
# 21 "linux-2.6.26/net/core/ethtool.c" 2
...
u32 ethtool_op_get_tx_csum(struct net_device *dev)
{
 return (dev->features & (((4 | 8) | 2) | ((4 | 8) | 16))) != 0;
}
...
u32 ethtool_op_get_tso(struct net_device *dev)
{
 return (dev->features & (SKB_GSO_TCPV4 << 16)) != 0;
}
...
u32 ethtool_op_get_ufo(struct net_device *dev)
{
 return (dev->features & (SKB_GSO_UDP << 16)) != 0;
}
...
typedef  const u32   _GLOBAL_144_T; static  _GLOBAL_144_T  global_flags_dup_features[NUM_STACKS] = {  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  ETH_FLAG_LRO,  };     
...
u32 ethtool_op_get_flags(struct net_device *dev)
{
 ...
 return dev->features & global_flags_dup_features[get_stack_id()];
}
...
{
 struct ethtool_cmd cmd = { 0x00000001 };
 int err;
...
static int ethtool_get_regs(struct net_device *dev, char *useraddr)
{
 ...
 regbuf = kmalloc(reglen, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u) | (( gfp_t)0x20000u)));
 if (!regbuf)
  return -12;
 ...
 if (copy_to_user(useraddr, &regs, sizeof(regs)))
  goto out;
 useraddr += __builtin_offsetof(struct ethtool_regs,data);
...
static int ethtool_get_eeprom(struct net_device *dev, void *useraddr)
{
 ...
 const struct ethtool_ops *ops = dev->ethtool_ops;
 void *userbuf = useraddr + sizeof(eeprom);
 u32 bytes_remaining;
 u8 *data;
 int ret = 0;
 ...
 data = kmalloc(((1UL) << 12), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u) | (( gfp_t)0x20000u)));
 if (!data)
  return -12;

 bytes_remaining = eeprom.len;
 while (bytes_remaining > 0) {
  eeprom.len = ({ typeof(bytes_remaining) _min1 = (bytes_remaining); typeof((u32)((1UL) << 12)) _min2 = ((u32)((1UL) << 12)); (void) (&_min1 == &_min2); _min1 < _min2 ? _min1 : _min2; });
...
{
 struct ethtool_coalesce coalesce = { 0x0000000e };
...
{
 struct ethtool_ringparam ringparam = { 0x00000010 };
...
{
 struct ethtool_pauseparam pauseparam = { 0x00000012 };
...
static int ethtool_set_tso(struct net_device *dev, char *useraddr)
{
 ...
 if (edata.data && !(dev->features & 1))
  return -22;

 return dev->ethtool_ops->set_tso(dev, edata.data);
}

static int ethtool_set_ufo(struct net_device *dev, char *useraddr)
{
 struct ethtool_value edata;
 ...
 if (edata.data && !(dev->features & 1))
  return -22;
 if (edata.data && !(dev->features & 8))
  return -22;
 return dev->ethtool_ops->set_ufo(dev, edata.data);
}

static int ethtool_get_gso(struct net_device *dev, char *useraddr)
{
 struct ethtool_value edata = { 0x00000023 };

 edata.data = dev->features & 2048;
 if (copy_to_user(useraddr, &edata, sizeof(edata)))
   return -14;
 return 0;
}

static int ethtool_set_gso(struct net_device *dev, char *useraddr)
{
 struct ethtool_value edata;
...
static int ethtool_self_test(struct net_device *dev, char *useraddr)
{
 ...
 if (test_len < 0)
  return test_len;
 ({ int __ret_warn_on = !!(test_len == 0); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_on_slowpath("linux-2.6.26/net/core/ethtool.c", 591); __builtin_expect(!!(__ret_warn_on), 0); });

 if (copy_from_user(&test, useraddr, sizeof(test)))
  return -14;

 test.len = test_len;
 data = kmalloc(test_len * sizeof(u64), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u) | (( gfp_t)0x20000u)));
 if (!data)
  return -12;
...
static int ethtool_get_strings(struct net_device *dev, void *useraddr)
{
 ...
 data = kmalloc(gstrings.len * 32, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u) | (( gfp_t)0x20000u)));
 if (!data)
  return -12;
 ...
 if (copy_to_user(useraddr, data, gstrings.len * 32))
  goto out;
 ret = 0;
...
static int ethtool_get_stats(struct net_device *dev, void *useraddr)
{
 ...
 if (n_stats < 0)
  return n_stats;
 ({ int __ret_warn_on = !!(n_stats == 0); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_on_slowpath("linux-2.6.26/net/core/ethtool.c", 705); __builtin_expect(!!(__ret_warn_on), 0); });

 if (copy_from_user(&stats, useraddr, sizeof(stats)))
  return -14;

 stats.n_stats = n_stats;
 data = kmalloc(n_stats * sizeof(u64), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u) | (( gfp_t)0x20000u)));
 if (!data)
  return -12;
...
static int ethtool_set_value_void(struct net_device *dev, char *useraddr,
...
int dev_ethtool(struct net *net, struct ifreq *ifr)
{
 ...
 void *useraddr = ifr->ifr_ifru.ifru_data;
 u32 ethcmd;
 int rc;
