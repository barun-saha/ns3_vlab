# 1 "linux-2.6.26/net/core/filter.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.26/net/core/filter.c"
# 19 "linux-2.6.26/net/core/filter.c"
...
{ }
# 20 "linux-2.6.26/net/core/filter.c" 2
...
# 22 "linux-2.6.26/net/core/filter.c" 2
...
# 23 "linux-2.6.26/net/core/filter.c" 2
...
# 24 "linux-2.6.26/net/core/filter.c" 2
...
# 27 "linux-2.6.26/net/core/filter.c" 2
...
int ipv4_doint_and_flush_strategy(ctl_table *table, int *name, int nlen,
      void *oldval, size_t *oldlenp,
      void *newval, size_t newlen);
# 29 "linux-2.6.26/net/core/filter.c" 2
...
# 30 "linux-2.6.26/net/core/filter.c" 2
...
# 1 "linux-2.6.26/include/asm/uaccess.h" 1
# 37 "linux-2.6.26/net/core/filter.c" 2
# 1 "linux-2.6.26/include/asm/unaligned.h" 1
...
# 1 "linux-2.6.26/include/linux/unaligned/access_ok.h" 1
...
{
 *((__be16 *)p) = (( __be16)__fswab16((val)));
}

static inline  __attribute__((always_inline)) void put_unaligned_be32(u32 val, void *p)
{
 *((__be32 *)p) = (( __be32)__fswab32((val)));
}

static inline  __attribute__((always_inline)) void put_unaligned_be64(u64 val, void *p)
{
 *((__be64 *)p) = (( __be64)__fswab64((val)));
}
# 9 "linux-2.6.26/include/asm/unaligned.h" 2
# 1 "linux-2.6.26/include/linux/unaligned/generic.h" 1
...
# 10 "linux-2.6.26/include/asm/unaligned.h" 2
# 38 "linux-2.6.26/net/core/filter.c" 2
...
static void *__load_pointer(struct sk_buff *skb, int k)
{
 ...
 if (k >= (-0x100000))
  ptr = skb_network_header(skb) + k - (-0x100000);
 else if (k >= (-0x200000))
  ptr = skb_mac_header(skb) + k - (-0x200000);
...
static inline  __attribute__((always_inline)) void *load_pointer(struct sk_buff *skb, int k,
...
# 79 "linux-2.6.26/net/core/filter.c"
...
;
# 112 "linux-2.6.26/net/core/filter.c"
...
load_b:
  ...
  case 0x03:
   ...
   continue;
  default:
   ({ int __ret_warn_on = !!(1); if (__builtin_expect(!!(__ret_warn_on), 0)) warn_on_slowpath("linux-2.6.26/net/core/filter.c", 288); __builtin_expect(!!(__ret_warn_on), 0); });
   return 0;
  }
  ...
  switch (k-(-0x1000)) {
  case 0:
   A = __fswab16(( __u16)(__be16)(skb->protocol));
   continue;
  case 4:
   A = skb->pkt_type;
   continue;
...
;
# 345 "linux-2.6.26/net/core/filter.c"
int sk_chk_filter(struct sock_filter *filter, int flen)
{
 ...
 return (((filter[flen - 1].code) & 0x07) == 0x06) ? 0 : -22;
}
;
...
{
 struct sk_filter *fp = ({ const typeof( ((struct sk_filter *)0)->rcu ) *__mptr = (rcu); (struct sk_filter *)( (char *)__mptr - __builtin_offsetof(struct sk_filter,rcu) );});
...
# 471 "linux-2.6.26/net/core/filter.c"
int sk_attach_filter(struct sock_fprog *fprog, struct sock *sk)
{
 ...
 fp = sock_kmalloc(sk, fsize+sizeof(*fp), ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
 if (!fp)
  return -12;
 ...
 do { local_bh_disable(); (void)0; do { } while (0); } while (0);
 old_fp = ({ typeof(sk->sk_filter) _________p1 = (*(volatile typeof(sk->sk_filter) *)&(sk->sk_filter)); do { } while (0); (_________p1); });
 ({ if (!__builtin_constant_p(fp) || ((fp) != ((void *)0))) __asm__ __volatile__("": : :"memory"); (sk->sk_filter) = (fp); });
 do { do { } while (0); (void)0; local_bh_enable(); } while (0);
...
int sk_detach_filter(struct sock *sk)
{
 ...
 filter = ({ typeof(sk->sk_filter) _________p1 = (*(volatile typeof(sk->sk_filter) *)&(sk->sk_filter)); do { } while (0); (_________p1); });
 if (filter) {
  ({ if (!__builtin_constant_p(((void *)0)) || ((((void *)0)) != ((void *)0))) __asm__ __volatile__("": : :"memory"); (sk->sk_filter) = (((void *)0)); });
