# 1 "linux-2.6.26/net/core/stream.c"
...
# 1 "<command-line>" 2
# 1 "linux-2.6.26/net/core/stream.c"
# 15 "linux-2.6.26/net/core/stream.c"
...
{ }
# 16 "linux-2.6.26/net/core/stream.c" 2
...
# 17 "linux-2.6.26/net/core/stream.c" 2
...
# 18 "linux-2.6.26/net/core/stream.c" 2
...
typedef  const struct in6_addr    _GLOBAL_83_T; extern  _GLOBAL_83_T  global_in6addr_linklocal_allnodes[NUM_STACKS];     


typedef  const struct in6_addr    _GLOBAL_84_T; extern  _GLOBAL_84_T  global_in6addr_linklocal_allrouters[NUM_STACKS];     
...
# 19 "linux-2.6.26/net/core/stream.c" 2
# 28 "linux-2.6.26/net/core/stream.c"
void sk_stream_write_space(struct sock *sk)
{
 struct socket *sock = sk->sk_socket;

 if (sk_stream_wspace(sk) >= sk_stream_min_wspace(sk) && sock) {
  clear_bit(2, &sock->flags);

  if (sk->sk_sleep && waitqueue_active(sk->sk_sleep))
   __wake_up(sk->sk_sleep, 1, 1, ((void *)0));
  if (sock->fasync_list && !(sk->sk_shutdown & 2))
   sock_wake_async(sock, SOCK_WAKE_SPACE, ((2 << 16)|2));
 }
}

;
# 51 "linux-2.6.26/net/core/stream.c"
int sk_stream_wait_connect(struct sock *sk, long *timeo_p)
{
 struct task_struct *tsk = nsc_get_task();
 ...
 do {
  ...
  sk->sk_write_pending++;
  done = ({ int __rc; release_sock(sk); __rc = !sk->sk_err && !((1 << sk->__sk_common.skc_state) & ~(TCPF_ESTABLISHED | TCPF_CLOSE_WAIT)); if (!__rc) { *(timeo_p) = schedule_timeout(*(timeo_p)); } lock_sock(sk); __rc = !sk->sk_err && !((1 << sk->__sk_common.skc_state) & ~(TCPF_ESTABLISHED | TCPF_CLOSE_WAIT)); __rc; });
...
static inline  __attribute__((always_inline)) int sk_stream_closing(struct sock *sk)
{
 return (1 << sk->__sk_common.skc_state) &
 ...
 if (timeout) {
  ...
  do {
   prepare_to_wait(sk->sk_sleep, &wait,
     1);
   if (({ int __rc; release_sock(sk); __rc = !sk_stream_closing(sk); if (!__rc) { *(&timeout) = schedule_timeout(*(&timeout)); } lock_sock(sk); __rc = !sk_stream_closing(sk); __rc; }))
    break;
  } while (!signal_pending(nsc_get_task()) && timeout);
...
int sk_stream_wait_memory(struct sock *sk, long *timeo_p)
{
 ...
 if (sk_stream_memory_free(sk))
  current_timeo = vm_wait = (random32() % (1000 / 5)) + 2;
 ...
 while (1) {
  ...
  sk->sk_write_pending++;
  ({ int __rc; release_sock(sk); __rc = !sk->sk_err && !(sk->sk_shutdown & 2) && sk_stream_memory_free(sk) && vm_wait; if (!__rc) { *(&current_timeo) = schedule_timeout(*(&current_timeo)); } lock_sock(sk); __rc = !sk->sk_err && !(sk->sk_shutdown & 2) && sk_stream_memory_free(sk) && vm_wait; __rc; });
...
{
 if (err == -32)
  err = sock_error(sk) ? : -32;
 if (err == -32 && !(flags & 0x4000))
  send_sig(13, nsc_get_task(), 0);
 return err;
}
...
void sk_stream_kill_queues(struct sock *sk)
{
 ...
 do { if (__builtin_expect(!!(!(skb_queue_empty(&sk->sk_write_queue))), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "skb_queue_empty(&sk->sk_write_queue)", "linux-2.6.26/net/core/stream.c" , 195); } } while(0);


 sk_mem_reclaim(sk);

 do { if (__builtin_expect(!!(!(!sk->sk_wmem_queued)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "!sk->sk_wmem_queued", "linux-2.6.26/net/core/stream.c" , 200); } } while(0);
 do { if (__builtin_expect(!!(!(!sk->sk_forward_alloc)), 0)) { printk("<3>" "KERNEL: assertion (%s) failed at %s (%d)\n", "!sk->sk_forward_alloc", "linux-2.6.26/net/core/stream.c" , 201); } } while(0);
